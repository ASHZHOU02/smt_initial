...
# YW: Logging/Debugging in Perl
use Log::Log4perl qw(:easy);
# DEBUG->INFO->WARN->ERROR->FATAL
Log::Log4perl->easy_init($DEBUG);
my $logger = Log::Log4perl->get_logger("");
# YW: matching elements in an array
use experimental 'smartmatch';
# YW: extracting min val from a list of num
use List::Util qw(min);
#TODO, change info/debug/error for all printing messages
...
# YW: Additional parameters
my $TOP_EXTPIN_PARAMETER = 0;  # ARGV[19], 0; Disable top I/O 1: Enable top I/O
...
if ($ARGC != 1) {
	...
	# read ENVAR
	$infile             = $ARGV[0];
	$BoundaryCondition  = int($ENV{BoundaryCondition});
	$SON                = int($ENV{SON});
	$DoublePowerRail    = int($ENV{DoublePowerRail});
	$MAR_Parameter      = int($ENV{MAR_Parameter});
	$EOL_Parameter      = int($ENV{EOL_Parameter});
	$VR_Parameter       = int($ENV{VR_Parameter});
	$PRL_Parameter      = int($ENV{PRL_Parameter});
	$SHR_Parameter      = int($ENV{SHR_Parameter});
	$MPL_Parameter       = int($ENV{MPL_Parameter});
	$MM_Parameter       = int($ENV{MM_Parameter});
	$Local_Parameter       = int($ENV{Local_Parameter});
	$Partition_Parameter       = int($ENV{Partition_Parameter});
	$BCP_Parameter       = int($ENV{BCP_Parameter});
	$NDE_Parameter       = int($ENV{NDE_Parameter});
	$BS_Parameter       = int($ENV{BS_Parameter});
	$PE_Parameter = int($ENV{PE_Parameter});
	$M2_TRACK_Parameter = int($ENV{M2_TRACK_Parameter});
	$M2_Length_Parameter = int($ENV{M2_Length_Parameter});
	$dint = int($ENV{DINT});
...
# ******************* YW: used for std cell dim estimation *****************
...
my $maxNumPTrackV = 0;					# Width of Placement Clip with M1 Metal Pitch
my $maxNumPTrackH = 0;					# Width of Placement Clip with M1 Metal Pitch
# **************************************************************************
...
my %map_metal_to_rows = ();
my %map_metal_to_cols = ();
# [METAL] => [row/col] => {0,1}
# my %map_metal_to_col_skip_factor = ();
# my %map_metal_to_row_skip_factor = ();
my %map_metal_to_rows_to_idx= ();
my %map_metal_to_cols_to_idx= ();
# originally, dint is 2 times M1 pitch, we keep the same concept here
# dint_idx is the factor of M1 pitch (for iteration purpose)
my $dint_idx = $dint;
# dint is the real distance in terms of M1 pitch 
$dint = $dint * $mapMP{3};

sub combine;
sub combine_sub;

# used to merge distint cols/rows for different metal layers so that we can iterate through all possible col and rows
# does not sort array
sub uniq (@) {
    my %h;
    map { $h{$_}++ == 0 ? $_ : () } @_;
...
# trying to shrink the transistor (by reducing the number of fingers) and put them on the same cols
# extending width but not length
sub getAvailableNumFinger{
	...
	for my $i(0 .. $trackEachPRow-1){
		# find compatiable length of fingers and exit immediately
...
# print("!getAvailableNumFinger:");
# print(getAvailableNumFinger(4, 2));
# print("\n");
...
sub combine {
	...
	# vectorization:  if n <= 1, each item instantiate a list
...
#print "$i $pins[$i][0] $pins[$i][1] $pins[$i][2] $pins[$i][7]\n";
	...
	if ($infileStatus eq "inst") {
		if ($line =~ /^i   ins(\S+)\s*(\S+)\s*(\d+)/) {	
			...
			# Example
			# [getAvailableNumFinger]: 8/(2-0)
			# $VAR1 = [
			# 	'4'
			# ];
			# $tmp_finger[$#tmp_finger]*2+1 is the full width of transistor 
			# 0 ... 4
			# iterate thru width of the transistor
			for my $i(0 .. ($tmp_finger[$#tmp_finger]*2+1)-1){
				if($i==0){
					$h_pin_idx{$instName} = $pinIDX;
				}
				# store possible pin (Y) row location
				@pinYpos = ();
				# pin row from 1..2
				...
				push (@pins, [@pin]);
				# Example: pinMM0_0, pinMM0_1 ... pinMM0_4
...
#print "$instName => pin$1_$i, , t, $trackEachPRow, $i, [@pinYpos], $instName, $pin_type\n";
	...
	# print("h_pin_idx:\n");
	# print(Dumper\%h_pin_idx);

	# print("h_pinId_idx:\n");
	# print(Dumper\%h_pinId_idx);
...
$logger->info("********** Standard Cell Dimension **********\n");
### Approximate Standard Cell Dimension Estimation
# YW: Width of sdc is constrained by placement layer
# $std_width = $mapOffSet{"1"} + $mapMP{"1"} * ($numPTrackV - 1);
$std_width = $mapOffSet{"1"} + $mapMP{"1"} * ($numTrackV);
# $std_height = $mapOffSet{"2"} + $mapMP{"2"} * $numPTrackH;
$std_height = $mapOffSet{"4"} + $mapMP{"4"} * ($numTrackH); # user define
$logger->info("	Width: $std_width \n");
$logger->info("	Height: $std_height \n");
$logger->info("*********************************************\n");
...
$logger->info("DDA : numPowerPmos :$numPowerPmos numPowerNmos :$numPowerNmos\n");
...
$logger->info("a     # Pins              = $totalPins\n");
$logger->info("a     # Nets              = $totalNets\n");
...
# YW: encode odd/even cols/rows
my $row_skip_factor = 0;
my $col_skip_factor = 0;
my %temp_map_row_to_skip_factor = ();
my %temp_map_col_to_skip_factor = ();
# store rows/cols to map skip factor
my @temp_rows = ();
my @temp_cols = ();
...
for my $metal (1 .. $numMetalLayer) {  # Odd Layers: Vertical Direction   Even Layers: Horizontal Direction
	...
	if ($metal == 1) {
		...
		$map_numTrackV{$metal} = scalar @temp_cols;
		$map_numTrackH{$metal} = scalar @temp_rows;

		# map metal rows/cols (excluding pwr rail)
		$map_metal_to_rows{$metal} = [@temp_rows[0..$#temp_rows-2]];
		$map_metal_to_cols{$metal} = [@temp_cols];
		...
		my %temp_map_row_to_idx = ();
		my %temp_map_col_to_idx = ();

		# assign row --> row_idx
		for my $i (0 .. $#temp_rows-2) {
		...
		# NOTE: back slash = a reference to the hash
		$map_metal_to_rows_to_idx{$metal} = {%temp_map_row_to_idx};
		$map_metal_to_cols_to_idx{$metal} = {%temp_map_col_to_idx};
		...
		# should include power rails
		$map_numTrackV{$metal} = scalar @temp_cols;
		$map_numTrackH{$metal} = scalar @temp_rows;

		# map metal rows/cols
		# should only relate to real vertices (no power rail)
		$map_metal_to_rows{$metal} = [@temp_rows[0..$#temp_rows-2]];
		$map_metal_to_cols{$metal} = [@temp_cols];
		...
		while($row < $std_height) {
			...
			while ($upCol < $std_width) {
				# if not
				unless ($upCol ~~ @temp_cols){
					push(@temp_cols, $upCol);
				}
				# jump to next col
				$upCol += $upMP; 
			}
			...
			while ($loCol < $std_width) {
				# if not
				if (not $loCol ~~ @temp_cols){
					push(@temp_cols, $loCol);
				}
		...
		# sort rows/cols so we can iterate properly
		@temp_rows = (sort { $a <=> $b } @temp_rows);
		@temp_cols = (sort { $a <=> $b } @temp_cols);
		...
		while($col < $std_width) {
			...
			while ($upRow < $std_height) {
				unless ($upRow ~~ @temp_rows){
					push(@temp_rows, $upRow);
				}
			...
			while ($loRow < $std_height) {
				# if not
				unless ($loRow ~~ @temp_rows){
					push(@temp_rows, $loRow);
				}
...
for my $metal (1 .. $numMetalLayer) {
	# store layer specific vertices
	my @temp_vertices = ();
	my $temp_numTrackH = $map_numTrackH{$metal};
	for my $row_idx (0 .. $temp_numTrackH-3) { # horiztonal dir
		my $row = $map_metal_to_rows{$metal}[$row_idx];
		my $temp_numTrackV = $map_numTrackV{$metal};
		for my $col_idx (0 .. $temp_numTrackV-1) { # vertical dir
			my $col = $map_metal_to_cols{$metal}[$col_idx];
			$vName = "m".$metal."r".$row."c".$col;
			if ($col_idx == 0) { ### Left Vertex
				$vL = "null";
			} 
			else {
				$vL = "m".$metal."r".$row."c".($map_metal_to_cols{$metal}[$col_idx-1]);
			}
			if ($col_idx == $temp_numTrackV-1) { ### Right Vertex
				$vR = "null";
			}
			else {
				$vR = "m".$metal."r".$row."c".($map_metal_to_cols{$metal}[$col_idx+1]);
			}
			if ($row_idx == 0) { ### Front Vertex
				$vF = "null";
			}
			else {
				$vF = "m".$metal."r".($map_metal_to_rows{$metal}[$row_idx-1])."c".$col;
			}
			if ($row_idx == $temp_numTrackH-3) { ### Back Vertex
				$vB = "null";
			}
			else {
				$vB = "m".$metal."r".($map_metal_to_rows{$metal}[$row_idx+1])."c".$col;
			}
			if ($metal == $numMetalLayer) { ### Up Vertex
				$vU = "null";
			}
			else {
				if ($row ~~ $map_metal_to_rows{$metal+1} && $col ~~ $map_metal_to_cols{$metal+1}){
			...
			if ($row_idx == 0 || $col_idx == 0) { ### FL Vertex
				$vFL = "null";
			}
			else {
				$vFL = "m".$metal."r".($map_metal_to_rows{$metal}[$row_idx-1])."c".($map_metal_to_cols{$metal}[$col_idx-1]);
			}
			if ($row_idx == 0 || $col_idx == $temp_numTrackV-1) { ### FR Vertex
				$vFR = "null";
			}
			else {
				$vFR = "m".$metal."r".($map_metal_to_rows{$metal}[$row_idx-1])."c".($map_metal_to_cols{$metal}[$col_idx+1]);
			}
			if ($row_idx == $temp_numTrackH-3 || $col_idx == 0) { ### BL Vertex
				$vBL = "null";
			}
			else {
				$vBL = "m".$metal."r".($map_metal_to_rows{$metal}[$row_idx+1])."c".($map_metal_to_cols{$metal}[$col_idx-1]);
			}
			if ($row_idx == $temp_numTrackH-3 || $col_idx == $temp_numTrackV-1) { ### BR Vertex
				$vBR = "null";
			}
			else {
				$vBR = "m".$metal."r".($map_metal_to_rows{$metal}[$row_idx+1])."c".($map_metal_to_cols{$metal}[$col_idx+1]);
			}
...
if (1) {
	...
	$logger->debug(Dumper(\%vertices));
	$logger->debug("**** map_numTrackV:\n");
	$logger->debug(Dumper(\%map_numTrackV));
	$logger->debug("**** map_numTrackH:\n");
	$logger->debug(Dumper(\%map_numTrackH));
	$logger->debug("**** map_metal_to_vertices:\n");
	$logger->debug(Dumper(\%map_metal_to_vertices));

	$logger->debug("**** map_metal_to_rows_to_idx:\n");
	$logger->debug(Dumper(\%map_metal_to_rows_to_idx));

	$logger->debug("**** map_metal_to_cols_to_idx:\n");
	$logger->debug(Dumper(\%map_metal_to_cols_to_idx));

	$logger->debug("**** map_metal_to_cols:\n");
	$logger->debug(Dumper(\%map_metal_to_cols));

	$logger->debug("**** map_metal_to_rows:\n");
	$logger->debug(Dumper(\%map_metal_to_rows));

	$logger->debug("**** std_width:\n");
	$logger->debug(Dumper(\$std_width));

	$logger->debug("**** std_height:\n");
	$logger->debug(Dumper(\$std_height));
}
...
my $vCost = 4;
my $mCost = 1;
my $vCost_1 = 4;	# Via Cost for layer 1
my $mCost_1 = 1;	# Metal Cost for layer 1
...
# Debug: for better reference, instead of iterating through vertices
# just interate through numTrackV and numTrackH like before
for my $metal (1 .. $numMetalLayer) {
	my $temp_numTrackH = $map_numTrackH{$metal};
	for my $row_idx (0 .. $temp_numTrackH-3){
		my $row = $map_metal_to_rows{$metal}[$row_idx];
		# bug: why this 
		my $temp_numTrackV = $map_numTrackV{$metal};
		for my $col_idx (0 .. $temp_numTrackV-1) {
			my $col = $map_metal_to_cols{$metal}[$col_idx];
			...
			if ($metal % 2 == 0) { # Even Layers ==> Horizontal
				...
				# 1/22 disable col % 2 for now
				# if($metal > 1 && $col_idx % 2 == 1){
				# 	next;
				# }
...
# YW: if we use M2 as well
# YW: We also assume top-1 layer is M3 and top layer is M4

# YW: Second Top layer = M3
	...
	for my $row_idx (0 .. $temp_numTrackH-3) {
		...
		for my $col_idx (0 .. $temp_numTrackV-1) {
			...
			if($metal%2==0) { # odd layer: vertical --> col
				...
				if($EXT_Parameter == 0){
					# YW: originally first two rows are mapped, now we still take first two rows
					if ($row_idx == 1 || $row_idx == $temp_numTrackH-4) {
...
for my $pinID (0 .. $#pins) {
	# $PIN_LENGTH is -1 when pin is connecting to VDD/VSS or ABY
	if ($pins[$pinID][3] == -1) {
		$commodityInfo = -1;
		# Initializing
...
for my $metal (1 .. $numMetalLayer) { # At the top-most metal layer, only vias exist.
	...
	for my $row_idx (0 .. $temp_numTrackH-3) {
		...
		for my $col_idx (0 .. $temp_numTrackV-1) {
			...
			# 1/22 disable odd col
			# elsif($metal % 2 == 1 && $col_idx % 2 == 1){
			# 	next;
			# }
	...
	if ($pins[$pinID][2] eq "s") { # source
		...
		if(exists $sources{$pins[$pinID][0]}){
			...
			if(exists($h_inst_idx{$pins[$pinID][6]})){
				...
				# these are merely indices, not real coordinate as we want
				my $ru_idx = $h_RTrack{0};				# upper --> 3
				my $rl_idx = $h_RTrack{$numPTrackH-1};	# lower --> 0
				#for my $row (0 .. $numTrackH/2-2){
				# bc we are comparing with real rows, we need to convert to real rows
				my $ru = $map_metal_to_rows{1}[$ru_idx];
				my $rl = $map_metal_to_rows{1}[$rl_idx];

				my $temp_numTrackH = $map_numTrackH{1};
				for my $row_idx (0 .. $temp_numTrackH-3){
					my $row = $map_metal_to_rows{1}[$row_idx];
					# for my $col (0 .. $numTrackV-1) {    
					my $temp_numTrackV = $map_numTrackV{1};
        			for my $col_idx (0 .. $temp_numTrackV-1){
						my $col = $map_metal_to_cols{1}[$col_idx];
						if(exists($h_mapTrack{$row_idx}) && $row_idx<=$ru_idx&& $row_idx>=$rl_idx){
							if($pins[$pinID][7] eq "G" && $col_idx%2 == 1){
								next;
							}
							elsif($pins[$pinID][7] ne "G" && $col_idx%2 == 0){
								next;
							}
		...
		if(exists $sinks{$pins[$pinID][0]}){
			...
			if($pins[$pinID][0] eq $keySON){
				...
				for my $term (0 ..  $sinks{$pins[$pinID][0]}[1]-1){
					# mapping subnodes of SON to pin
					@virtualEdge = ($vEdgeIndex, $sinks{$pins[$pinID][0]}[2][$term], $pins[$pinID][0], $virtualCost);
					push (@virtualEdges, [@virtualEdge]);
					$vEdgeIndex++;
				}
			}
			# if pin related instance ID existed in h_inst_idx (same as above)
...
print("\nedge_in:\n");
print(length %edge_in);

print("\nedge_out:\n");
print(length %edge_out);

print("\nvedge_in:\n");
print(length %vedge_in);

print("\nvedge_out:\n");
print(length %vedge_out);
...
### Z3 Option Set ###
print $out "(set-option :smt.core.minimize true)\n";
...
# print "$max_numTrackV, $max_key_V, $max_numTrackH, $max_key_H\n";
# exit;
...
# M2 here is top layer M4
for my $i (0 .. $map_numTrackH{4}-3){
	my $trk = $map_metal_to_rows{4}[$i];
	print $out "(declare-const M2_TRACK_$trk Bool)\n";
}
# M2 here is top layer M4
foreach my $key(keys %h_extnets){
	for my $i (0 .. $map_numTrackH{4}-3){
		# convert to real track
		my $trk = $map_metal_to_rows{4}[$i];
		print $out "(declare-const N".$key."_M2_TRACK_$trk Bool)\n";
	}
...
# YW: asserting legal instance location along x axis
...
for my $i (0 .. $numInstance - 1) {
	...
	$len2 = length(sprintf("%b", $maxNumPTrackV - (2*$tmp_finger[0] + 1) * $mapMP{"1"}));
	my $tmp_str = "";
	if($len>1){
		for my $i(0 .. $len-$len2-1){
			$tmp_str.="0";
	...
	# never used
	my $s_second = "#b".$tmp_str.sprintf("%b", ($maxNumPTrackV - 2*$tmp_finger[0]*$mapMP{"1"}));
	# print $out "(assert (and (bvuge x$i $s_first) (bvule x$i $s_second)))\n";
	# YW: asserting legal instance location along x axis
	# 	* Signed $len bit compare instance i x_pos >= 0
	# 	* AND
	# 	* Signed $len bit compare instance i x_pos <= numPTrack Vertical - 2 finger - 1
	print $out "; $tmp_finger[0], $maxNumPTrackV\n";
	print $out "(assert (! (and (bvsge x$i (_ bv0 $len)) (bvsle x$i (_ bv".($maxNumPTrackV - (2*$tmp_finger[0] + 1) * $mapMP{"1"})." $len))) :named c$CONSTRAINT_ID))\n";
	$CONSTRAINT_ID += 1;
	cnt("l", 0);
	cnt("l", 0);
	cnt("c", 0);
...
# PMOS Y axis variable, number-of-finger variable, width variable, unit width variable
for my $i (0 .. $lastIdxPMOS) {
	my @tmp_finger = ();
	@tmp_finger = getAvailableNumFinger($inst[$i][2], $trackEachPRow);
	#print $out "(assert (= y$i (_ bv".($numPTrackH-$inst[$i][2]/$tmp_finger[0])." ".length(sprintf("%b", $numPTrackH)).")))\n";
	print $out "(assert (! (= y$i (_ bv0 ".length(sprintf("%b", $numPTrackH)).")) :named c$CONSTRAINT_ID))\n"; # CFET y coordinate overlapping
	$CONSTRAINT_ID += 1;
	cnt("l", 0);
	cnt("c", 0);
	print $out "(assert (! (= nf$i (_ bv".$tmp_finger[0]." ".length(sprintf("%b", $tmp_finger[0])).")) :named c$CONSTRAINT_ID))\n";
	$CONSTRAINT_ID += 1;
	cnt("l", 0);
	cnt("c", 0);
	print $out "(assert (! (= uw$i (_ bv".$inst[$i][2]/$tmp_finger[0]." ".(length(sprintf("%b", $trackEachPRow))).")) :named c$CONSTRAINT_ID))\n";
	$CONSTRAINT_ID += 1;
	cnt("l", 0);
	cnt("c", 0);
	print $out "(assert (! (= w$i (_ bv".(2*$tmp_finger[0]+1)." ".length(sprintf("%b", 2*$tmp_finger[0]+1)).")) :named c$CONSTRAINT_ID))\n";
	$CONSTRAINT_ID += 1;
	cnt("l", 0);
	cnt("c", 0);
}

# NMOS Y axis variable, number-of-finger variable, width variable, unit width variable (same as above)
...
# 	# All instances x position's first bit should be equal to value 1?
# 	# 1/7 this means x1 must be a odd number,
# 	# this however is not 
...
my $tmp_minWidth = 0;
# minWidth is max(PMOS width, NMOS width)
# PMOS width = 2 * num-of-fingers
for my $i (0 .. $lastIdxPMOS) {
	my @tmp_finger = ();
	@tmp_finger = getAvailableNumFinger($inst[$i][2], $trackEachPRow);
...
$tmp_minWidth = 0;
# NMOS width = 2 * num-of-fingers (same as above)
for my $i ($lastIdxPMOS + 1 .. $numInstance - 1) {
	my @tmp_finger = ();
	@tmp_finger = getAvailableNumFinger($inst[$i][2], $trackEachPRow);
	$tmp_minWidth+=2*$tmp_finger[0];
...
if($BS_Parameter == 1){
	...
	print "numPMOS : $numPMOS  numNMOS : $numNMOS  numInstance: $numInstance\n";
	my @arr_pmos = ();
	my @arr_nmos = ();
	# arr_pmos = PMOS indices
	for my $i (0 .. $lastIdxPMOS){
		push(@arr_pmos, $i);
	}
	print("arr_pmos: $numPMOS\n");
	print(Dumper\@arr_pmos);
	# arr_nmos = NMOS indices
	for my $i ($lastIdxPMOS + 1 .. $numInstance - 1) {
		push(@arr_nmos, $i);
	}
	print("arr_nmos: $numNMOS\n");
	print(Dumper\@arr_nmos);
	my @comb_l_pmos = ();
	my @comb_l_nmos = ();
	my @comb_c_pmos = ();
	my @comb_c_nmos = ();
	...
	if($numPMOS % 2 == 0){
		# if even number of PMOS
		my @tmp_comb_l_pmos = combine([@arr_pmos],$numPMOS/2);
		print("tmp_comb_l_pmos\n");
		print(Dumper\@tmp_comb_l_pmos);
	...
	for my $i(0 .. $#comb_l_pmos){
		...
		print $out "):named c$CONSTRAINT_ID))\n";
		$CONSTRAINT_ID += 1;
		#print $out " (and";
	...
	for my $i (0 .. $lastIdxPMOS) {
		...
		@tmp_finger = getAvailableNumFinger($inst[$i][2], $trackEachPRow);
		if($tmp_finger[0]%2==0){
			print $out "(assert (! (= ff$i false) :named c$CONSTRAINT_ID))\n";
			$CONSTRAINT_ID += 1;
			cnt("l", 0);
			cnt("c", 0);
		}
...
for my $i (0 .. $lastIdxPMOS) {
	...
	if($inst[$i][2]/$tmp_finger[0] == 1){
		# if width/finger == 1
		# gate_pmos_unit length 1 <= instance #i
		push(@g_p_h1, $i);
		# total width of pmos in unit length 1
		$w_p_h1+=2*$tmp_finger[0];
		# hashing gate instance #i = unit length 1
		$h_g_inst{$i} = 1;
	}
	# unit width == 2
	elsif($inst[$i][2]/$tmp_finger[0] == 2){
		# if width/finger == 2
		# gate_pmos_unit length 2 <= instance #i
		push(@g_p_h2, $i);
		# width of pmos in unit length 2
		$w_p_h2+=2*$tmp_finger[0];
		# hashing gate instance #i = unit length 2
		$h_g_inst{$i} = 2;
	}
	# unit width == 3
	elsif($inst[$i][2]/$tmp_finger[0] == 3){
		# if width/finger == 3
		# gate_pmos_unit length 3 <= instance #i
		push(@g_p_h3, $i);
		# width of pmos in unit length 3
		$w_p_h3+=2*$tmp_finger[0];
		# hashing gate instance #i = unit length 3
		$h_g_inst{$i} = 3;
	}
}
...
for my $i ($lastIdxPMOS + 1 .. $numInstance - 1) {
	...
	if($inst[$i][2]/$tmp_finger[0] == 1){
		# if width/finger == 1
		# gate_nmos_unit length 1 <= instance #i
		push(@g_n_h1, $i);
		# width of nmos in unit length 1
		...
		# if width/finger == 2
		# gate_nmos_unit length 2 <= instance #i
		push(@g_n_h2, $i);
		# width of nmos in unit length 2
		...
		# if width/finger == 3
		# gate_nmos_unit length 3 <= instance #i
		push(@g_n_h3, $i);
		# width of nmos in unit length 3
...
# If finger is even, both ends of mosFET are the same S or same D, so flipping is not applicable
# If finger is odd, flipping might be applied
# Between every two PMOS
for my $i (0 .. $lastIdxPMOS) {
	for my $j (0 .. $lastIdxPMOS) {
		if($i != $j){
			# netID of Source and Drain pin
			...
			# if both instance fingers are even
			if($tmp_finger_i[0] % 2 == 0 && $tmp_finger_j[0] % 2 == 0){
				# the net info should be the same in bitvector
				$tmp_str_ij = "(= (_ bv$tmp_key_S_i ".length(sprintf("%b", $numNets_org)).") (_ bv$tmp_key_S_j ".length(sprintf("%b", $numNets_org))."))";
				# should this be j before i?
				$tmp_str_ji = "(= (_ bv$tmp_key_S_i ".length(sprintf("%b", $numNets_org)).") (_ bv$tmp_key_S_j ".length(sprintf("%b", $numNets_org))."))";
			}
			# if instance i is even finger, instance j is odd finger
			elsif($tmp_finger_i[0] % 2 == 0 && $tmp_finger_j[0] % 2 == 1){
				# if nf % 2 == 1, if ff = 1 nl = $tmp_key_D, nr = $tmp_key_S
				#                 if ff = 0 nl = $tmp_key_S, nr = $tmp_key_D
				# nri = nlj
				
				# if net info on instance i Source is the same as instance j Drain
				if($tmp_key_S_i == $tmp_key_D_j){
					# if net info on instance i Source is the same instance j Source
					if($tmp_key_S_i == $tmp_key_S_j){
						# implies net info on instance j Drain and Source are the same, flipping is not applicable
						$tmp_str_ij = "";
					}
					else{
						# instance j flip flap = true
						$tmp_str_ij = "(= ff$j true)";
					}
				}
				# if net info on instance i Source is diff from instance j Drain
				else{
					# if net info on instance i Source is the same instance j Source
					if($tmp_key_S_i == $tmp_key_S_j){
			...
			# if instance i is odd finger, instance j is even finger
...
#				$tmp_str_ji = $tmp_str_ji." (= $tmp_key_S_i $tmp_key_D_j))))";
			...
			if(($height_i == $height_j) && ($tmp_key_S_i == $tmp_key_S_j || $tmp_key_S_i == $tmp_key_D_j || $tmp_key_D_i == $tmp_key_S_j || $tmp_key_D_i == $tmp_key_D_j)){
				...
				print $out "	    (= #b1 #b0))))) :named c$CONSTRAINT_ID))\n";
				$CONSTRAINT_ID += 1;
				cnt("l", 0);
				cnt("l", 0);
				cnt("l", 0);
...
if ($stack_struct_flag eq "PN") {
	...
	for my $vEdgeIndex (0 .. $#virtualEdges) {
		...
		# YW: NMOS
		# NOTE: Assumption this is 4 track so row from 0~3
		# 2/2 should replace with last row
		if($instIdx > $lastIdxPMOS 
			&& ($map_metal_to_rows_to_idx{$toMetal}{$toRow} != 0 
			&& $map_metal_to_rows_to_idx{$toMetal}{$toRow} != 3 ) 
			&& ($map_metal_to_cols_to_idx{$toMetal}{$toCol} > 0 
			&& $map_metal_to_cols_to_idx{$toMetal}{$toCol} <= $map_metal_to_cols_to_idx{$toMetal}{$map_numTrackV{$toMetal}-2}) 
			&& ($virtualEdges[$vEdgeIndex][2] ne "pinSON")) {
...
print("PMOS_forbidden:\n");
print(Dumper\%PMOS_forbidden);
print("NMOS_forbidden:\n");
print(Dumper\%NMOS_forbidden);
...
#				print $out "(assert (= M_$rightCorners[$rightVertex]_RightEnd false))\n";
	...
	for my $frontVertex (0 .. $#frontCorners) {
		...
		if ($metal % 2 == 1) {	# only on odd metal 1, 3, 5...
			...
			if ($virtualEdges[$vEdgeIndex][2] =~ /^pin/) { # source
				if ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3]){
					if($pins[$h_pinId_idx{$nets[$netIndex][3]}][7] eq "G"){ ### GATE Pin
						my $metal = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[1]; # need metal for mapping factor
						my $col = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3]; # 1:metal 2:row 3:col
						if($map_metal_to_cols_to_idx{$metal}{$col} % 2 == 0){
...
#							print $out "(assert (= N$nets[$netIndex][1]_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false))\n";
						...
						my $metal = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[1]; # need metal for mapping factor
						my $col = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3]; # 1:metal 2:row 3:col
						if($map_metal_to_cols_to_idx{$metal}{$col} % 2 == 1){
...
if($Local_Parameter == 1){
	...
	for my $netIndex (0 .. $#nets) {
		for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
			...
			my $len = length(sprintf("%b", $max_numTrackV))+4;
			# NOT USED
			$pidx_s =~ s/pin\S+_(\d+)/\1/g; # =~ applying a regular expression to a scalar.
			$pidx_t =~ s/pin\S+_(\d+)/\1/g;
			my %h_edge = (); # will always be empty ???

			# if curr commodity in sinks_inNet is not an SON
			if($nets[$netIndex][5][$commodityIndex] ne $keySON){
				# if source pin and sink pin are the same
					...
					for my $metal (3 .. $numMetalLayer) {
						...
						for my $col_idx (0 .. $temp_numTrackV-1) {
							...
							for my $row_idx (0 .. $temp_numTrackH-3) {
								...
								# 1/22 disable odd col
								# if($metal>1 && $metal % 2 == 1 && $col_idx % 2 == 1){
								# 	next;
								# }
...
while($isEnd == 0){
	...
	print $out ";Set Default Gate Metal according to the capacity variables\n";
	### Set Default Gate Metal according to the capacity variables
	my %h_tmp = ();
	# 0 ... net index
	# $logger->debug("$#nets, $#virtualEdges\n");
	# exit;
	for my $netIndex (0 .. $#nets) {
		# 0 ... numSinks - 1
		# $logger->debug("netloop, $netIndex\n");
		for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
			# total virtual edges
			# $logger->debug("commodityIndexloop, $commodityIndex\n");
			for my $vEdgeIndex (0 .. $#virtualEdges) {
				# $logger->debug("vedgeloop, $vEdgeIndex\n");
				my $tmp_vname = "";
				my $tmp_vname1 = "";
				my $tmp_vname2 = "";
				# if virtualedge destination starts with "pin"
				if ($virtualEdges[$vEdgeIndex][2] =~ /^pin/){ # source
					# if virtual edge destination is the source of net
					if ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3]){
						# retrieve instance of source pin of net
						my $instIdx = $h_inst_idx{$pins[$h_pinId_idx{$nets[$netIndex][3]}][6]};
						my @tmp_finger = ();
						# compute finger of this instance
						@tmp_finger = getAvailableNumFinger($inst[$instIdx][2], $trackEachPRow);
						# NOTE: Assumption this is 4 track so row from 0~3
						# 1/22 future implementation should change this to numTrack used
						my $height = 4;  # Mark: What is this use for? Used for different structure in ISCAS paper
						
						# if source pin of the net pin type is GATE
						if($pins[$h_pinId_idx{$nets[$netIndex][3]}][7] eq "G"){ ### GATE Pin
							# extract origin vertex of vEdge
							my $metal = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[1]; # 1:metal 2:row 3:col
							my $col = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3]; # 1:metal 2:row 3:col
							# YW: no longer need to loop since we have a datastruture
							my $col_idx = $map_metal_to_cols_to_idx{$metal}{$col};
							#if($col % 2 == 0 && ($col == 0 || $col == $numTrackV-1)){
							# if origin vertex's column is even
							if($map_metal_to_cols_to_idx{$metal}{$col} % 2 == 0 ){
								# extract origin vertex of vEdge
								$tmp_vname = $virtualEdges[$vEdgeIndex][1];
								$tmp_vname1 = $tmp_vname;
								$tmp_vname2 = $tmp_vname;
								# search and replace m#r#c# --> ve_N#_C#_E_r#c#_pin#
								# N[NetID]
								# C[numSinks#]
								# pin[Destination]
								# print("before tmp_vname: $tmp_vname\n");
								$tmp_vname =~ s/m[0-9](.*)/ve_N$nets[$netIndex][1]\_C$commodityIndex\_E_\1_$virtualEdges[$vEdgeIndex][2]/g;
								# print("after tmp_vname: $tmp_vname\n");
								# if PMOS
								if($instIdx <= $lastIdxPMOS){
									# 0 .. 2
									for my $i(0 .. $height-2){
										...
										#	next;
										#}
										# '=~' operator is a binary binding operator that indicates the following operation will search or modify the scalar on the left
										print("before tmp_vname: $tmp_vname1\n");
										$tmp_vname1 =~ s/m(\d+)r(\d+)c(\d+)/m\1r$j\\c\3/g;
										print("mid tmp_vname: $tmp_vname1\n");
										$tmp_vname1 =~ s/\\//g;
										print("after tmp_vname: $tmp_vname1\n");
										$tmp_vname2 =~ s/m(\d+)r(\d+)c(\d+)/m\1r$k\\c\3/g;
										$tmp_vname2 =~ s/\\//g;
										# only a print, ignore $numTrackV
										...
										if(!exists($h_tmp{$tmp_vname1.$tmp_vname2})){
											if(!exists($h_assign{"M_$tmp_vname1\_$tmp_vname2"})){
												...
												my $len = length(sprintf("%b", $max_numTrackV))+4;
												# need to check if col is minimum
												if ($col_idx == 0) {
													# ($col>0?($col-1):$col): if col is greater than 0, then (col - 1), else col
													# if bitvector signed COST SIZE >= col
													# then assert M_v#_vR# = true
													# else do nothing?
													# YW: we need a way to refer col back to col_idx?
													$str.="(assert (! (ite (bvsge COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 true) (= 1 1)):named c$CONSTRAINT_ID))\n";
													$CONSTRAINT_ID += 1;
												} else {
													# if bitvector signed COST SIZE >= col
													# then assert M_v#_vR# = false
													# else do nothing?
													# $str.="(assert (ite (bvslt COST_SIZE (_ bv".($col_idx>0?($col-1):$col)." $len)) (= M_$tmp_vname1\_$tmp_vname2 false) (= 1 1)))\n";
													$str.="(assert (! (ite (bvslt COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 false) (= 1 1)):named c$CONSTRAINT_ID))\n";
													$CONSTRAINT_ID += 1;
												}
								...
								# if NMOS (same as above)
									...
									for my $i(0 .. $height-2){
										...
										$tmp_vname2 =~ s/m(\d+)r(\d+)c(\d+)/m\1r$k\\c\3/g;
										$tmp_vname2 =~ s/\\//g;
										# YW: this is only a print, ignore $numTrackV
										# print "$col/$numTrackV: COST_SIZE: M_$tmp_vname1\_$tmp_vname2\n";
										if(!exists($h_tmp{$tmp_vname1.$tmp_vname2})){
											if(!exists($h_assign{"M_$tmp_vname1\_$tmp_vname2"})){
												setVar("M_$tmp_vname1\_$tmp_vname2", 2);
					...
					# if virtual edge destination is the one of the sinks# of the net
					elsif ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]){
						# if virtual edge destination is pinSON
						if(!($virtualEdges[$vEdgeIndex][2] eq $keySON)){
							# extract instance index of the sink pin of the net 
							my $instIdx = $h_inst_idx{$pins[$h_pinId_idx{$nets[$netIndex][5][$commodityIndex]}][6]};
							my @tmp_finger = ();
							# compute finger of this instance
							@tmp_finger = getAvailableNumFinger($inst[$instIdx][2], $trackEachPRow);
							my $height = 3;
							# if the sink pin of the net pin type is GATE
							if($pins[$h_pinId_idx{$nets[$netIndex][5][$commodityIndex]}][7] eq "G"){ ### GATE Pin
								my $metal = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[1]; # 1:metal 2:row 3:col
								my $col = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3]; # 1:metal 2:row 3:col
								...
								# Mark : Also need to modify it
									...
									$tmp_vname2 = $tmp_vname;
									$tmp_vname =~ s/m[0-9](.*)/ve_N$nets[$netIndex][1]\_C$commodityIndex\_E_\1_$virtualEdges[$vEdgeIndex][2]/g;
									# if PMOS (same as above)
									if($instIdx <= $lastIdxPMOS){
										# NOTE: $height-2 because we assume PTrack to be 4-2=2 rows
										for my $i(0 .. $height-2){
											my $j=0+$i;
											my $k=$j+1;
											#if (($col != 0 && $col != $numTrackV-1) && ( $i == 0 || $i == $height-2)) {
											...
											if(!exists($h_tmp{$tmp_vname1.$tmp_vname2})){
												if(!exists($h_assign{"M_$tmp_vname1\_$tmp_vname2"})){
													...
													if ($col_idx == 0) {
														...
														# then assert M_v#_vR# = true
														# else do nothing?
														print("(assert (!(ite (bvsge COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 true) (= 1 1)):named c$CONSTRAINT_ID))\n");
														$str.="(assert (!(ite (bvsge COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 true) (= 1 1)):named c$CONSTRAINT_ID))\n";
														$CONSTRAINT_ID += 1;
													} else {
														# if bitvector signed COST SIZE >= col
														# then assert M_v#_vR# = false
														# else do nothing?
														print("(assert (!(ite (bvslt COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 false) (= 1 1)):named c$CONSTRAINT_ID))\n");
														$str.="(assert (!(ite (bvslt COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 false) (= 1 1)):named c$CONSTRAINT_ID))\n";
														$CONSTRAINT_ID += 1;
													}
													$h_tmp{$tmp_vname1.$tmp_vname2} = 1;
	...
	$str.=";0.1.Unset All Metal/Net/Wire over the rightmost cell/metal(>COST_SIZE+1)\n";
	# Unset All Metal/Net/Wire over the rightmost cell/metal(>COST_SIZE+1)
	# for each undirected edges
	for my $udeIndex (0 .. $#udEdges) {
		# extract metal/row/col from both vertices
		...
		my $toCol_idx = $map_metal_to_cols_to_idx{$toMetal}{$toCol};
		# set the length by max num of tracks
		my $len = length(sprintf("%b", $max_numTrackV))+4;
		# if rightmost col is more than minwidth
		if($minWidth>2 && $toCol_idx > $minWidth-2){
			# if not Metal M_#vertex1_#vertex2
			if(!exists($h_assign{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
				setVar("M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]", 2);
				# if COST_SIZE <= toCol - 2:
				# then M_#vertex1_#vertex2 = False
				# else, do nothing
				$str.="(assert (! (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] false) (= true true)) :named c$CONSTRAINT_ID))\n";
				$CONSTRAINT_ID += 1;
				cnt("l", 1);
				cnt("l", 1);
				cnt("c", 1);
	...
	for my $netIndex (0 .. $#nets) {
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			# if rightmost col is more than minwidth
			if($minWidth>2 && $toCol_idx > $minWidth-2){
				# if not Net #NETID_E_#vertex1_#vertex2 
				if(!exists($h_assign{"N$nets[$netIndex][1]\_E_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					# print("DEBUG: N$nets[$netIndex][1]\_E_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]\n");
					setVar("N$nets[$netIndex][1]\_E_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]", 2);
					# if COST_SIZE <= toCol - 2:
					# then Net NETID_E_#vertex1_#vertex2 = False
					# else, do nothing
					$str.="(assert (! (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= N$nets[$netIndex][1]\_";
					$str.="E_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] false) (= true true)):named c$CONSTRAINT_ID))\n";
					$CONSTRAINT_ID += 1;
					cnt("l", 1);
					cnt("l", 1);
					cnt("c", 1);
	...
	# for each net
	for my $netIndex (0 .. $#nets) {
		# for each undirected commodity flow index
		for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
			for my $udeIndex (0 .. $#udEdges) {
				my $fromCol = (split /[a-z]/, $udEdges[$udeIndex][1])[3]; # 1:metal 2:row 3:col
				my $toCol   = (split /[a-z]/, $udEdges[$udeIndex][2])[3];
				...
				# if rightmost col is more than minwidth
				if($minWidth>2 && $toCol_idx > $minWidth-2){
					# if not Net/Commodity #NETID C#commodityflow E_#vertex1_#vertex2
					if(!exists($h_assign{"N$nets[$netIndex][1]\_C$commodityIndex\_E_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
						...
						# if COST_SIZE <= toCol - 2:
						# then Net/Commodity #NETID C#commodityflow E_#vertex1_#vertex2 = False
						# else, do nothing
						$str.="(assert (! (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= N$nets[$netIndex][1]\_";
	...
	# for each virtual edges
	for my $vEdgeIndex (0 .. $#virtualEdges) {
		# extract origin vertex col
		...
		if($minWidth>2 && $toCol_idx > $minWidth-2){
			...
			if(!exists($h_assign{"M_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"})){
				...
				# if COST_SIZE <= toCol - 2:
				# then Metal M_#vertex1_#vertex2 = False
				# else, do nothing
				$str.="(assert (! (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= M_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)):named c$CONSTRAINT_ID))\n";
				$CONSTRAINT_ID += 1;
				cnt("l", 1);
				cnt("l", 1);
				cnt("c", 1);
	...
	for my $netIndex (0 .. $#nets) {
		...
		for my $vEdgeIndex (0 .. $#virtualEdges) {
			...
			# if rightmost col is more than minwidth
			if($minWidth>2 && $toCol_idx > $minWidth-2){
				# if virtual edge destination vertex is the same as the net source_ofNet
				if($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3]){
					# if not Net NETID_E_#vertex1_#vertex2
					if(!exists($h_assign{"N$nets[$netIndex][1]_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"})){
						...
						# then Net NETID_E_#vertex1_#vertex2 = False
						# else, do nothing
						$str.="(assert (! (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= N$nets[$netIndex][1]_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
						cnt("l", 1);
						cnt("l", 1);
						cnt("c", 1);
				...
				# if virtual edge destination vertex is different than the net source_ofNet
				else{
					# for each commodity flow
					for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
						# if virtual edge destination vertex equals to any of the sink of the net
	...
	for my $netIndex (0 .. $#nets) {
		# for each virtual edges
		for my $vEdgeIndex (0 .. $#virtualEdges) {
			# extract col from origin vertex
			my $toMetal   = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[1];
			my $toCol   = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3];
			my $toCol_idx = $map_metal_to_cols_to_idx{$toMetal}{$toCol};
			# YW: ignored next 2 lines for some reason
			...
			if($minWidth>2 && $toCol_idx > $minWidth-2){
				...
				if($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3]){
					...
					for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
						# if virtual edge destination vertex equals to any of the sink of the net
						if($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]){
							# # if not Net NETID C#commodityflow E_#vertex1_#vertex2
							if(!exists($h_assign{"N$nets[$netIndex][1]_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"})){
								...
								# minus 2 because SDB?
								$str.="(assert (! (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= N$nets[$netIndex][1]_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)):named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("l", 1);
								cnt("l", 1);
								cnt("c", 1);
	...
	print("numTrackAssign: $numTrackAssign \n");

	# YW: [ignore] will not be used since we dont have net track
	if($numTrackAssign>0){
				...
				if($toMetal == 4){
					...
					if(exists($h_net_track_n{$nets[$netIndex][1]}) && !exists($h_net_track{"$nets[$netIndex][1]\_$fromRow"})){
						if(!exists($h_assign{"N$nets[$netIndex][1]\_E_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
							...
							$str.="E_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] false):named c$CONSTRAINT_ID))\n";
							$CONSTRAINT_ID += 1;
							cnt("l", 1);
							cnt("l", 1);
							cnt("c", 1);
			...
			if(exists($h_track_net{$row})){
				...
				if($#track_nets>0){
					...
					for my $udeIndex (0 .. $#udEdges) {
						...
						if($cnt_var>1){
							$str.="(assert (!((_ at-most 1)$tmp_str):named c$CONSTRAINT_ID))\n";
							$CONSTRAINT_ID += 1;
						}
					}
				}
			...
			my $j = $i + 1;
				for my $k(0 .. $#{$inst_group_p[$i][1]}){
					for my $l(0 .. $#{$inst_group_p[$j][1]}){
						$str.="(assert (!(bvsgt x$inst_group_p[$i][1][$k] x$inst_group_p[$j][1][$l]):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
			...
			my $j = $i + 1;
				for my $k(0 .. $#{$inst_group_n[$i][1]}){
					for my $l(0 .. $#{$inst_group_n[$j][1]}){
						$str.="(assert (!(bvsgt x$inst_group_n[$i][1][$k] x$inst_group_n[$j][1][$l]):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
	...
	if($Partition_Parameter == 1){
		...
		if($numOutP > 0 && $numInP > 0){
			for my $i(0 .. $numOutP-1){
				for my $j(0 .. $numInP-1){
					$str.="(assert (!(bvsgt x$arr_out_p[$i] x$arr_in_p[$j]):named c$CONSTRAINT_ID))\n";
					$CONSTRAINT_ID += 1;
					cnt("l", 0);
					cnt("l", 0);
					cnt("c", 0);
		...
		if($numOutN > 0 && $numInN > 0){
			for my $i(0 .. $numOutN-1){
				for my $j(0 .. $numInN-1){
					$str.="(assert (!(bvsgt x$arr_out_n[$i] x$arr_in_n[$j]):named c$CONSTRAINT_ID))\n";
					$CONSTRAINT_ID += 1;
					cnt("l", 0);
					cnt("l", 0);
					cnt("c", 0);
		...
		# TODO, what is this?
		my $def_offset = int($max_numTrackV/1.7);
		for my $netIndex (0 .. $#nets) {
			my $instIdx_s = 0;
			my @tmp_finger_s = ();
			my $width_s = 0;
	...
	$str.=";0.2 Exculde More than 2 Nets in one column on M1 to improve pin accesibility\n";
	# change col to M1 specific
	# for each col at M1
	foreach (@{$map_metal_to_cols{3}}) {
		my $col = $_;
		# set valid = 0
		my $valid = 0;
		# assert AM1
		...
		for my $netIndex (0 .. $#nets) {
			...
			for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {	
				...
				for my $vEdgeIndex (0 .. $#virtualEdges) {
					# extract col of the origin vertex
					my $toMetal = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[1]; # 1:metal 2:row 3:col
					my $toCol = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3]; # 1:metal 2:row 3:col
					# correponding col index
					my $toCol_idx = $map_metal_to_cols_to_idx{$toMetal}{$toCol};
					my $len = length(sprintf("%b", $max_numTrackV))+4;
					# if destination vertex is SON && at origin vertex col && (destination vertex is net source_ofNet || destination vertex is any of the net sink)
					if ($virtualEdges[$vEdgeIndex][2] eq "pinSON" && $col == $toCol && ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3] || $virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]) ) {
						# print("where $virtualEdges[$vEdgeIndex][1], $virtualEdges[$vEdgeIndex][2] \n");
						# N_C_E_#vertex1_#vertex2
	...
	# $VAR1 = [
			...
			#   12
			# ];
	# print($map_metal_to_cols{3}[2]);
	# print($map_numTrackV{3});
	...
	if ($PE_Parameter == 1) { 
		...
		if ($PE1_newflag == 0) {
			# for ease of implementation, we simply iterate col index
			my $temp_numTrackV = $map_numTrackV{3};
			for my $col_idx (0 .. $temp_numTrackV-1){
				my $col = $map_metal_to_cols{3}[$col_idx];
				my $valid = 0;
				my $len = length(sprintf("%b", $max_numTrackV))+4;
				#my $tmp_str="(assert (ite (and (bvsge COST_SIZE (_ bv".$col_idx." $len)) (or ";
				#my $tmp_str="(assert (ite (bvsge COST_SIZE (_ bv".$col_idx." $len)) (ite (and (or ";
				...
				if ( $dint_idx -2 < $col_idx ) { # assume -2 has one pin
					#$tmp_str.=") (and ";
					my @tmp_arr = ();
					for my $netIndex (0 .. $#nets) {
						#$tmp_str.="(and ";
						...
						for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {	
							for my $vEdgeIndex (0 .. $#virtualEdges) {
								...
								my $toCol_idx = $map_metal_to_cols_to_idx{$toMetal}{$toCol};
								my $len = length(sprintf("%b", $max_numTrackV))+4;
								#if ($virtualEdges[$vEdgeIndex][2] eq "pinSON" && ( ($col_idx - $toCol == 2) || ($toCol - $col_idx == 2) ) && ($col_idx != 0) && ($toCol != 0) && ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3] || $virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]) ) {
								if ($virtualEdges[$vEdgeIndex][2] eq "pinSON" && ( ($col_idx - $toCol_idx == $dint) || ($toCol_idx - $col_idx == $dint) ) && ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3] || $virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]) ) {
									# print("where $virtualEdges[$vEdgeIndex][1], $virtualEdges[$vEdgeIndex][2] \n");
									my $tmp_var = "N$nets[$netIndex][1]\_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]";
									push (@tmp_arr, $tmp_var);
					...
					#$tmp_str.=") ";
					$tmp_str.=") (= COST_Pin_C$col true) (= COST_Pin_C$col false) ):named c$CONSTRAINT_ID))\n";
					$CONSTRAINT_ID += 1;
				} else { # pin interfered by boundary
					$tmp_str.=")) (= COST_Pin_C$col false) (= COST_Pin_C$col false) ):named c$CONSTRAINT_ID))\n";
					$CONSTRAINT_ID += 1;
				}
		...
		if ($PE1_newflag == 1) { # PE=1 New edge based implementation for performance
			...
			for my $col_idx (0 .. $temp_numTrackV - 1){
				my $col = $map_metal_to_cols{3}[$col_idx];
				#print ("Edge-based pin separation: col $col_idx\n");
				# skipping odd
				# 1/22 disable odd col since it is on M3
				# next if ($col_idx % 2 == 1);
				...
				for my $netIndex (0 .. $#nets) {
					for my $extnetIndex (0 .. $#outerPins) {
						...
						if ($outerPins[$extnetIndex][1] eq $nets[$netIndex][0]) {
							...
							#print("Ext net found!\n");	
							#for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
								my $temp_numTrackH = $map_numTrackH{3};
								# for my $row (0 .. $numTrackH-4){
								# YW: do I need - 4?
								for my $row_idx (0 .. $temp_numTrackH-4){
									my $row = $map_metal_to_rows{3}[$row_idx];
									my $row_b = $map_metal_to_rows{3}[$row_idx+1];
									my $vName_1 = "m3r".$row."c".$col;
									my $vName_2 = "m3r".$row_b."c".$col;
									my $tmp_vname = "N$nets[$netIndex][1]_E_$vName_1\_$vName_2";
									# print("where $tmp_vname \n");
				...
				$tmp_str.=") ";
				if ($dint_idx - 2 < $col_idx) { # assume boundary have one pin 
					# right col
					$tmp_str.="(or ";
					# right aggcol
					# 1/22 bug found, empty clause
					# temp hold value if written
					my $right_written = 0;
					my $right_tmp_str = "";
					if ($col_idx < $temp_numTrackV-1) {
						$right_tmp_str.="(or ";
						my $aggcol_rightmost_idx = $col_idx + $dint_idx;
						if ($aggcol_rightmost_idx > $temp_numTrackV - 1) {
							$aggcol_rightmost_idx = $temp_numTrackV - 1;
						}
						for (my $aggcol_idx = $col_idx+$dint_idx; $aggcol_idx<=$aggcol_rightmost_idx; $aggcol_idx=$aggcol_idx+2) {
							my $aggcol = $map_metal_to_cols{3}[$aggcol_idx];
							for my $netIndex (0 .. $#nets) {
								for my $extnetIndex (0 .. $#outerPins) {
									# Check whether it is external pin
									# print ("Compare $outerPins[$extnetIndex][1] and $nets[$netIndex][0]\n");
									if ($outerPins[$extnetIndex][1] eq $nets[$netIndex][0]) {
										...
										# for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {	
											...
											for my $row_idx (0 .. $temp_numTrackH-4){
												...
												my $vName_2 = "m3r".$row_b."c".$aggcol;
												my $tmp_vname = "N$nets[$netIndex][1]_E_$vName_1\_$vName_2";
												# print("where $tmp_vname \n");
												$right_tmp_str.="(= $tmp_vname true) ";
												setVar($tmp_vname, 2);
											} # end row
										#} # end Commodity
									} # end if ext nets
						...
						if ($right_written == 1) {
							$right_tmp_str.=") ";
							# $tmp_str.="e) ";
							$tmp_str.=$right_tmp_str;
						}
					}
					
					# left aggcol
					# 2022/12/29 bug found, empty clause
					# temp hold value if written
					my $left_written = 0;
					my $left_tmp_str = "";
					if ($col_idx > 0) {
						# $tmp_str.="(or d";
						$left_tmp_str.="(or ";
						my $aggcol_leftmost_idx = $col_idx - $dint_idx;
						if ($aggcol_leftmost_idx < 0) {
							$aggcol_leftmost_idx = 0;
						}
						for (my $aggcol_idx = $aggcol_leftmost_idx; $aggcol_idx<$col_idx; $aggcol_idx=$aggcol_idx+2) {
							my $aggcol = $map_metal_to_cols{3}[$aggcol_idx];
							for my $netIndex (0 .. $#nets) {
								for my $extnetIndex (0 .. $#outerPins) {
									# Check whether it is external pin
						...
						if ($left_written == 1) {
							$left_tmp_str.=") ";
							# $tmp_str.="e) ";
							$tmp_str.=$left_tmp_str;
						}
						

					}
					$tmp_str.=")) (= $cost_name true) (= $cost_name false)) :named c$CONSTRAINT_ID))\n";
					$CONSTRAINT_ID += 1;
					cnt("l", 1);
					$str.=$tmp_str;
					setVar($cost_name, 2);
	...
	if ($PE_Parameter == 2 or $PE_Parameter == 3) { 
		...
		# for my $col (1 .. $numTrackV-1){
			...
			# 1/22 disable odd col since on M3
			# next if ($col_idx % 2 == 1);
			for my $netIndex (0 .. $#nets) {
				for my $extnetIndex (0 .. $#outerPins) {
					# Check whether it is external pin
					...
					if ($outerPins[$extnetIndex][1] eq $nets[$netIndex][0]) {
						...
						for my $row_idx (0 .. $temp_numTrackH-4){
							my $row = $map_metal_to_rows{3}[$row_idx];
							my $row_b = $map_metal_to_rows{3}[$row_idx + 1];
							my $row_bb = $map_metal_to_rows{3}[$row_idx + 2];
							my $row_f = $map_metal_to_rows{3}[$row_idx - 1];
							my $valid = 0;
							my $tmp_str="(assert (!(ite (and (or ";
							my $vName_1 = "m3r".$row."c".$col;
							my $vName_2 = "m3r".$row_b."c".$col;
							...
							for my $aggnetIndex (0 .. $#nets) {
								for my $aggextnetIndex (0 .. $#outerPins) {
									if ($outerPins[$aggextnetIndex][1] eq $nets[$aggnetIndex][0] &&  $nets[$aggnetIndex][0] ne $nets[$netIndex][0]) {
										if ($col_idx < $temp_numTrackV-1) {
											my $aggcol_rightmost_idx = $col_idx+$dint_idx;
											if ($aggcol_rightmost_idx > $temp_numTrackH-1 ) {
												$aggcol_rightmost_idx = $temp_numTrackH-1;
											}
											for (my $aggcol_idx = $col_idx+$dint_idx; $aggcol_idx <= $aggcol_rightmost_idx; $aggcol_idx = $aggcol_idx + 2) {
												$valid = 1;
												my $aggcol = $map_metal_to_cols{3}[$aggcol_idx];
												my $aggvName_1 = "m3r".$row."c".$aggcol;
												my $aggvName_2 = "m3r".$row_b."c".$aggcol;
												...
												if ($row_idx > 0) {
													$aggvName_1 = "m3r".$row_f."c".$aggcol;
													$aggvName_2 = "m3r".$row."c".$aggcol;
													$tmp_aggvname = "N$nets[$aggnetIndex][1]_E_$aggvName_1\_$aggvName_2";
													$tmp_str.="(= $tmp_aggvname true) ";
													setVar($tmp_aggvname, 2);
												}
												if ($row_idx < $temp_numTrackH-4) {
													$aggvName_1 = "m3r".$row_b."c".$aggcol;
													$aggvName_2 = "m3r".$row_bb."c".$aggcol;
													$tmp_aggvname = "N$nets[$aggnetIndex][1]_E_$aggvName_1\_$aggvName_2";
													$tmp_str.="(= $tmp_aggvname true) ";
													setVar($tmp_aggvname, 2);
												}
							...
							if ($valid == 1) {
								...
								$tmp_str .= "(= 1 1) )) (= $cost_name false) (= $cost_name false)):named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
							}
							cnt("l", 1);
							$str.=$tmp_str;
						...
						for my $row_idx (0 .. $temp_numTrackH-4){
							...
							# Check other net in $dint
							$tmp_str.=") (or ";
							if ($dint_idx - 2 < $col_idx ) { # assume boundary pin interference
							for my $aggnetIndex (0 .. $#nets) {
								print("PE23 $tmp_str\n");
								for my $aggextnetIndex (0 .. $#outerPins) {
									if ($outerPins[$aggextnetIndex][1] eq $nets[$aggnetIndex][0] &&  $nets[$aggnetIndex][0] ne $nets[$netIndex][0]) {
										if ($col_idx > 0 ) {
											...
											for (my $aggcol_idx = $aggcol_leftmost_idx; $aggcol_idx < $col_idx; $aggcol_idx = $aggcol_idx + 2) {
												$valid = 1;
												my $aggcol = $map_metal_to_rows{3}[$aggcol_idx];
												my $aggvName_1 = "m3r".$row."c".$aggcol;
												my $aggvName_2 = "m3r".$row_b."c".$aggcol;
												my $tmp_aggvname = "N$nets[$aggnetIndex][1]_E_$aggvName_1\_$aggvName_2";
												$tmp_str.="(= $tmp_aggvname true) ";
												...
												# 1/30 bug found: not using numTrack correctly
												if ($row_idx < $temp_numTrackH-4) {
													$aggvName_1 = "m3r".$row_b."c".$aggcol;
													$aggvName_2 = "m3r".$row_bb."c".$aggcol;
													$tmp_aggvname = "N$nets[$aggnetIndex][1]_E_$aggvName_1\_$aggvName_2";
							...
							if ($valid == 1) {
								...
								$tmp_str .= "(= 1 1))) (= $cost_name true) (= $cost_name false)):named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
							}
							cnt("l", 1);
							$str.=$tmp_str;
	...
	if ($PE_Parameter == 3) { 
		...
		for my $col_idx (0 .. $temp_numTrackV-1){
			...
			for my $netIndex (0 .. $#nets) {
				...
				for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {	
					for my $vEdgeIndex (0 .. $#virtualEdges) {
						...
						my $toCol   = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3];
						my $len = length(sprintf("%b", $max_numTrackV))+4;
						if ($virtualEdges[$vEdgeIndex][2] eq "pinSON" && $col_idx == $toCol && ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3] || $virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]) ) {
							my $tmp_var = "N$nets[$netIndex][1]\_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]";
							#push (@tmp_arr, $tmp_var);
							$tmp_str.=" (= $tmp_var true)";
							$valid = 1;
			...
			#$tmp_str.=") (and ";
			if ($dint_idx - 2 < $col_idx) { # assume boundary pin
				my @tmp_arr = ();
				for my $netIndex (0 .. $#nets) {
					#$tmp_str.="(and ";
					#my @tmp_arr = ();
				...
				for my $netIndex (0 .. $#nets) {
					for my $extnetIndex (0 .. $#outerPins) {
						...
						if ($outerPins[$extnetIndex][1] eq $nets[$netIndex][0]) {
							...
							#for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
								my $temp_numTrackH = $map_numTrackH{3};
								for my $row_idx (0 .. $temp_numTrackH-1){
									my $row = $map_metal_to_rows{$row_idx};
									my $row_b = $map_metal_to_rows{3}[$row_idx + 1];
									my $vName_1 = "m3r".$row."c".$col;
									my $vName_2 = "m3r".$row_b."c".$col;
									my $tmp_vname = "N$nets[$netIndex][1]_E_$vName_1\_$vName_2";
								...
								for my $extnetIndex (0 .. $#outerPins) {
									...
									if ($outerPins[$extnetIndex][1] eq $nets[$netIndex][0]) {
										...
										#for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {	
											...
											for my $row_idx (0 .. $temp_numTrackH-1){
												my $row = $map_metal_to_rows{$row_idx};
												my $row_b = $map_metal_to_rows{$row_idx+1};
												my $vName_1 = "m3r".$row."c".$aggcol;
												my $vName_2 = "m3r".$row_b."c".$aggcol;
												my $tmp_vname = "N$nets[$netIndex][1]_E_$vName_1\_$vName_2";
												$tmp_str.="(= $tmp_vname true) ";
	...
	$str.=";0.3 Set Initial Value for Shared Gate/S/D Source to sinks Pins of P/N FET in the same column\n";
	...
	for my $netIndex (0 .. $#nets) {
		for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {	
			...
			# YW: we should keep M1 horizontal tracks as constant = 4 for 4.5T?
			my $temp_numTrackH = $map_numTrackH{1};
			# use row index -> row for accurate vname
			$row_s_idx = 0;
			$row_s = $map_metal_to_rows{1}[$row_s_idx];
			$row_t_idx = $temp_numTrackH - 3;
			$row_t = $map_metal_to_rows{1}[$row_t_idx];
			...
			# YW: Since dealing with source and drain, assuming to be M1 cols
			my $temp_numTrackV = $map_numTrackV{1};
			# for my $col (1 .. $numTrackV-2){
			for my $col_idx (1 .. $temp_numTrackV-2){
				my $col = $map_metal_to_cols{1}[$col_idx];
				...
				my $cnt_var_T = 0;
				my $cnt_var_F = 0;
				if(($col_idx % 2 == 0 && $pins[$h_pinId_idx{$nets[$netIndex][3]}][7] eq "G") 
					|| ( $col_idx % 2 == 1 && ($pins[$h_pinId_idx{$nets[$netIndex][3]}][7] ne "G" 
					&& $pins[$h_pinId_idx{$nets[$netIndex][5][$commodityIndex]}][7] ne "G"))){
					...
					for my $row_idx (0 .. $temp_numTrackH-4){
						my $row = $map_metal_to_rows{1}[$row_idx];
						my $row_b = $map_metal_to_rows{1}[$row_idx+1];
						my $vName_1 = "m1r".$row."c".$col;
						my $vName_2 = "m1r".$row_b."c".$col;
					...
					if($cnt_var_T>0 || $cnt_var_F>0){
						if($col_idx % 2 == 0 && $col_idx-$tmp_pidx_s>0 && $col_idx-$tmp_pidx_t>0 && $pins[$h_pinId_idx{$nets[$netIndex][3]}][7] eq "G"){
						#if($col % 2 == 0  && $pins[$h_pinId_idx{$nets[$netIndex][3]}][7] eq "G"){
							# Shared gate case
							print "Shared Gate Case\n";
							$str.="(assert (ite";
							# encoding using col idx
							$str.=" (and (= x$instIdx_s (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_s)]." $len))";
							$str.=" (= x$instIdx_t (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_t)]." $len)))";
							cnt("l", 1);
							cnt("l", 1);
							$str.=" (and";
						...
						elsif ($col_idx % 2 == 1 && $pins[$h_pinId_idx{$nets[$netIndex][3]}][7] ne "G") {
							# Shared Source/Drain
							...
							my $isValid_t = 0;
							#$tmp_str.=" (or";
							if($tmp_pidx_s <= $col_idx){
								# Inst_s ff0 -> Inst_t ff0 or ff1
								$isValid_s = 1;
								$tmp_str.=" (and (= ff$instIdx_s false)";
								cnt("l", 1);
								# YW: will this be enough len?
								$tmp_str.=" (= x$instIdx_s (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_s)]." $len))";
								cnt("l", 1);
								$tmp_str.=" (or";
								if ($tmp_pidx_t <= $col_idx) {
									$isValid_t = 1;
									$tmp_str.=" (and (= ff$instIdx_t false)";
									cnt("l", 1);
									# YW: will this be enough len?
									$tmp_str.=" (= x$instIdx_t (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_t)]." $len)))";
									cnt("l", 1);
								}
								if (((($startIdx + $endIdx_t - 1 - $tmp_pidx_t)%2==0?($startIdx + $endIdx_t - 1 - $tmp_pidx_t):($startIdx + $endIdx_t - 1 - $tmp_pidx_t + 1))) <= $col_idx){
									if($isValid_t == 0){
									...
									my $len = length(sprintf("%b", $max_numTrackV))+4;
									# YW: will this be enough len?
									$tmp_str.=" (= x$instIdx_t (_ bv".$map_metal_to_cols{1}[($col_idx-((($startIdx + $endIdx_t - 1 - $tmp_pidx_t)%2==0?($startIdx + $endIdx_t - 1 - $tmp_pidx_t):($startIdx + $endIdx_t - 1 - $tmp_pidx_t + 1))))]." $len)))";
									cnt("l", 1);
								}
								$tmp_str.="))";	
							}
							...
							if (((($startIdx + $endIdx_s - 1 - $tmp_pidx_s)%2==0?($startIdx + $endIdx_s - 1 - $tmp_pidx_s):($startIdx + $endIdx_s - 1 - $tmp_pidx_s + 1))) <= $col_idx){
								...
								my $len = length(sprintf("%b", $max_numTrackV))+4;
								$tmp_str.=" (= x$instIdx_s (_ bv".$map_metal_to_cols{1}[($col_idx-((($startIdx + $endIdx_s - 1 - $tmp_pidx_s)%2==0?($startIdx + $endIdx_s - 1 - $tmp_pidx_s):($startIdx + $endIdx_s - 1 - $tmp_pidx_s + 1))))]." $len))";
								cnt("l", 1);
								$tmp_str.=" (or";
								if ($tmp_pidx_t <= $col_idx) {
									$isValid_t = 1;
	...
	print "=============================\n";	
	$str.=";0.4 Set Initial Value for Shared Gate/S/D Sink Pins of P/N FET in the same column\n";
	## Data Structure: pin = ($pinName, $pin_netID, $pinIO, $pinLength, $pinXpos, [@pinYpos], $pin_instID, $pin_type);
	# for all pins
	for my $pin_idx1 (0 .. $#pins) {
		# for all other pins (starting from current pin)
		for my $pin_idx2 ($pin_idx1+1 .. $#pins) {
			...
			if ( $nets[$netIndex1][0] eq $nets[$netIndex2][0] ) {
				...
				if( ($pins[$pin_idx1][7] eq "G" && $pins[$pin_idx2][7] eq "G") || ( ($pins[$pin_idx1][7] eq "D" || $pins[$pin_idx1][7] eq "S") && ($pins[$pin_idx2][7] eq "D" || $pins[$pin_idx2][7] eq "S") ) ) {
					...
					# for my $col (1 .. $numTrackV-2) {
					# YW: Assume it is on M1
					# iterate through all columns
					my $temp_numTrackV = $map_numTrackV{1};
					for my $col_idx (1 .. $temp_numTrackV-2){
						my $col = $map_metal_to_cols{1}[$col_idx];
						# Skip if both pin are gates but do not lie on even cols.
						# SKip if both pin are not gates but do not lie on odd cols.
						if((($pins[$pin_idx1][7] eq "G" && $pins[$pin_idx2][7] eq "G") && ($col_idx % 2 != 0)) || (($pins[$pin_idx1][7] ne "G" && $pins[$pin_idx2][7] ne "G") && ($col_idx % 2 == 0))) {
							next;
						}
						...
						my $len = length(sprintf("%b", $max_numTrackV))+4;
						#my $tmp_str = "";
						my @tmp_var_T1 = ();	# Storing Track variable
						my @tmp_var_F1 = ();	# NOT USED!!! Storing Flow variable
						my $cnt_var_T1 = 0;
						my $cnt_var_F1 = 0;
						# YW: extract M1 rows
						$row_s_idx = 0;
						...
						# YW: why is this necessary? This isnt even an array
						# print("netIndex1: $netIndex1\n");
						foreach ($netIndex1) {
							my $netIndex = $_;

							# Search the commidity flow pairs
							my @commodity_array = ();
							# Skip if either pin_name is the source_ofNet, because we only care about sink pins
							if (($pins[$pin_idx1][0] eq $nets[$netIndex][3]) ||($pins[$pin_idx2][0] eq $nets[$netIndex][3]) )	{
								...
								# iterate through commodity
								for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
									# Add commodityIndex to commodity_array if either pin is the same as sink pins
							...
							# YW: why commodity array size + 1 has to be 2? Is it because only two possible access point for sink pins (top and bottom)?
							...
							# iterate through commodity array
							foreach my $commodityIndex (@commodity_array) {
								...
								### YW: TGx1 is tristate gate with in/out/EN
								...
								for my $row_idx (0 .. $temp_numTrackH-4){
									...
									# up one row
									my $row_b = $map_metal_to_rows{1}[$row_idx + 1];
									# YW: do not confuse with outer column loop
									my $temp_numTrackV_2 = $map_numTrackV{1};
									# for my $tmp_col (1 .. $numTrackV-1) {
									# iterating through cols
									for my $col_idx_2 (0 .. $temp_numTrackV_2-1){
										my $tmp_col = $map_metal_to_cols{1}[$col_idx_2];

										my $vName_1 = "m1r".$row."c".$tmp_col;
										# my $vName_2 = "m1r".($row+1)."c".$tmp_col;
										# YW: need to consider MP
										my $vName_2 = "m1r".$row_b."c".$tmp_col;
										# Net Commodity Variable
										my $tmp_vname = "N$nets[$netIndex][1]_C$commodityIndex\_E_$vName_1\_$vName_2";
										# Track(metal) Variable
										my $tmp_vname1 = "M_$vName_1\_$vName_2";

										# store track variable only if outer col loop == inner col loop
										if ($tmp_col == $col) {
											print "$tmp_vname\n";
									...
									if($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex] && $virtualEdges[$vEdgeIndex][2] ne $keySON){
										if ($toCol == $col) {
											...
											if ( $vEdge_Instidx > $lastIdxPMOS ) {
												# print ("NMOS!!!\n");
												if ($stack_struct_flag eq "PN") {
													# YW: CAUTION this needs to be converted to GR specific location
													# assuming this is the first row
													if ($toRow == $map_metal_to_rows{1}[0]) {
													...
													# stack_struct_flag == "NP"
													# assuming this is the last row
													if ($toRow == $map_metal_to_rows{1}[-1]) {
														# NMOS virtual pin in case 1 ( 4 routing tracks case )
															push(@tmp_var_T1, $tmp_vname);
														setVar($tmp_vname, 2);
														$cnt_var_T1++;
							...
							# print(Dumper\@commodity_array);
						} # End Nets

						if($cnt_var_T1 >0 || $cnt_var_F1>0 ){
							# Check Flip
							if($col_idx % 2 == 0 && $col_idx-$tmp_pidx_1>0 && $col_idx-$tmp_pidx_2>0 && ($pins[$pin_idx1][7] eq "G" && $pins[$pin_idx2][7] eq "G") ){
								# Share gate case
								print "Share Sink Gate Case\n";
								$str.="(assert (!(ite";
								# this should be real col as it represents instance starting location
								$str.=" (and (= x$instIdx1 (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_1)]." $len))";
								$str.=" (= x$instIdx2 (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_2)]." $len)))";
								cnt("l", 1);
								cnt("l", 1);
								$str.=" (and";

								# verify track variable
								print("tmp_var_T1\n");
								print(Dumper\@tmp_var_T1);
								# verify flow variable
								print("tmp_var_F1\n");
								print(Dumper\@tmp_var_F1);
								...
								$str.=") (= 1 1)) :named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("c", 1);
							}
							elsif ($col_idx % 2 == 1 && ($pins[$pin_idx1][7] ne "G" && $pins[$pin_idx2][7] ne "G")) {
								# Share Source/Drain
								...
								my $isValid_2 = 0;
								#$tmp_str.=" (or";
								if($tmp_pidx_1 <= $col_idx){
									# Inst_1 ff0 -> Inst_2 ff0 or ff1
									$isValid_1 = 1;
									$tmp_str.=" (and (= ff$instIdx1 false)";
									cnt("l", 1);
									$tmp_str.=" (= x$instIdx1 (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_1)]." $len))";
									cnt("l", 1);
									$tmp_str.=" (or";
									if ($tmp_pidx_2 <= $col_idx) {
										$isValid_2 = 1;
										$tmp_str.=" (and (= ff$instIdx2 false)";
										cnt("l", 1);
										$tmp_str.=" (= x$instIdx2 (_ bv".$map_metal_to_cols{1}[($col_idx-$tmp_pidx_2)]." $len)))";
										cnt("l", 1);
									}
									if (((($startIdx + $endIdx_2 - 1 - $tmp_pidx_2)%2==0?($startIdx + $endIdx_2 - 1 - $tmp_pidx_2):($startIdx + $endIdx_2 - 1 - $tmp_pidx_2 + 1))) <= $col_idx){
										if($isValid_2 == 0){
										...
										my $len = length(sprintf("%b", $max_numTrackV))+4;
										$tmp_str.=" (= x$instIdx2 (_ bv".$map_metal_to_cols{1}[($col_idx-((($startIdx + $endIdx_2 - 1 - $tmp_pidx_2)%2==0?($startIdx + $endIdx_2 - 1 - $tmp_pidx_2):($startIdx + $endIdx_2 - 1 - $tmp_pidx_2 + 1))))]." $len)))";
										cnt("l", 1);
									}
									$tmp_str.="))";	
								}
								...
								if (((($startIdx + $endIdx_1 - 1 - $tmp_pidx_1)%2==0?($startIdx + $endIdx_1 - 1 - $tmp_pidx_1):($startIdx + $endIdx_1 - 1 - $tmp_pidx_1 + 1))) <= $col_idx){
									...
									my $len = length(sprintf("%b", $max_numTrackV))+4;
									$tmp_str.=" (= x$instIdx1 (_ bv".$map_metal_to_cols{1}[($col_idx-((($startIdx + $endIdx_1 - 1 - $tmp_pidx_1)%2==0?($startIdx + $endIdx_1 - 1 - $tmp_pidx_1):($startIdx + $endIdx_1 - 1 - $tmp_pidx_1 + 1))))]." $len))";
									cnt("l", 1);
									$tmp_str.=" (or";
									if ($tmp_pidx_2 <= $col_idx) {
										$isValid_2 = 1;
	...
	# DONE, BUT NEED MORE TEST

	$str.=";0.5 Set Initial Value for splitted G/S/D Pins of Each P/N FET in the same column\n";
		...
		for my $pin_idx2 ($pin_idx1+1 .. $#pins) {
			...
			if ( $nets[$netIndex1][0] ne $nets[$netIndex2][0] ) {
				...
				if( ($pins[$pin_idx1][7] eq "G" && $pins[$pin_idx2][7] eq "G") || ( ($pins[$pin_idx1][7] eq "D" || $pins[$pin_idx1][7] eq "S") && ($pins[$pin_idx2][7] eq "D" || $pins[$pin_idx2][7] eq "S") ) ) {
					...
					# for my $col (1 .. $numTrackV-2) {
					my $temp_numTrackV = $map_numTrackV{1};
					for my $col_idx (0 .. $temp_numTrackV-2){
						my $col = $map_metal_to_cols{1}[$col_idx];
						if((($pins[$pin_idx1][7] eq "G" && $pins[$pin_idx2][7] eq "G") && ($col_idx % 2 != 0)) 
							|| (($pins[$pin_idx1][7] ne "G" && $pins[$pin_idx2][7] ne "G") && ($col_idx % 2 == 0))) {
							next;										
						}
						...
						$row_s = 0;
						# 1/30 bug found: not using numTrack correctly
						$row_t = $map_numTrackH{1} - 3;
						my $tmp_pidx_1 = $pins[$pin_idx1][0];
						my $tmp_pidx_2 = $pins[$pin_idx2][0];
						$tmp_pidx_1 =~ s/pin\S+_(\d+)/\1/g;
						$tmp_pidx_2 =~ s/pin\S+_(\d+)/\1/g;

						# YW: why do you need this for loop to begin with???
						# You should use pairs-like data structure
						foreach ($netIndex1, $netIndex2) {
							...
							if ($netIndex == $netIndex1) {
								...
								if ($pins[$pin_idx1][0] eq $nets[$netIndex][3])	{
									# Matching pin with net Sources, add to commodity array
									print "$nets[$netIndex1][0]: source pin $pins[$pin_idx1][0]\n";
									for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
										push (@commodity_array, $commodityIndex);
									}
									...
									# Sinks
									print "$nets[$netIndex1][0]: sink pin $pins[$pin_idx1][0]\n";
									# Matching pin with net Sink, add to commodity array
									for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
									...
									if($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex] && $virtualEdges[$vEdgeIndex][2] ne $keySON){
										if ($toCol == $col) {
											...
											if ( $vEdge_Instidx > $lastIdxPMOS ) {
												# print ("NMOS!!!\n");
												if ($stack_struct_flag eq "PN") {
													# YW: Use real coordinate
													# if ($toRow < $numTrackH-3) { 	==> 4-3=1 then its only 0
													if ($toRow < $map_metal_to_rows{1}[$temp_numTrackH-3]) {
														# NMOS virtual pin in case 1 ( 4 routing tracks case )  
														print "F1: $tmp_vname\n";
														push(@tmp_var_F1, $tmp_vname);
														setVar($tmp_vname, 2);
													...
													# if ($toRow >= $numTrackH-4) {	==> 4-4=0
													if ($toRow >= $map_metal_to_rows{1}[$temp_numTrackH-4]) {
														# NMOS virtual pin in case 1 ( 4 routing tracks case )
														print "F1: $tmp_vname\n";
														push(@tmp_var_F1, $tmp_vname);
														setVar($tmp_vname, 2);
													...
													# if ($toRow <= 1) {		 ==> 4-3=1 then its only 0
													if ($toRow <= $map_metal_to_rows{1}[1]) {
														# NMOS virtual pin in case 2 ( 4 routing tracks case )
														print "F2: $tmp_vname\n";
														push(@tmp_var_F2, $tmp_vname);
	...
	# 2022/12/29 bug found
	# Split Dummy G/S/D
	$str.=";0.6 Set Initial Value for splitted Dummy G/S/D of Each P/N FET in the same column\n";
	if ($stack_struct_flag eq "PN") {
		$str.=";P on N struct\n";
		## Split Gate/S/D of PMOS and NMOS instance
		## Data Structure: pin = ($pinName, $pin_netID, $pinIO, $pinLength, $pinXpos, [@pinYpos], $pin_instID, $pin_type);
		...
		for my $col_idx (1 .. $temp_numTrackV-2){
			...
			for my $row_idx (0 .. $temp_numTrackH-4){
				...
				my $row_s = $row;
				# my $row_e = $row_s+1;
				my $row_e = $map_metal_to_rows{1}[$row_idx+1];
				my $vName_s = "m1r".$row_s."c".$col;
				my $vName_e = "m1r".$row_e."c".$col;
				my $tmp_vname = "M_$vName_s\_$vName_e";
				if (($dummy_g_struct_flag eq "Split" && $col_idx%2 == 0) || ( $dummy_sd_struct_flag eq "Split" && $col_idx%2 == 1 )) {
					# use real coordinate
					# if ($row_s == $numTrackH-4) {
					if ($row_s == $map_metal_to_rows{1}[$temp_numTrackH - 4]) {
						push (@tmp_var_false1, $tmp_vname);
					}
					# elsif ($row_s >= 0) {
					elsif ($row_s >= $map_metal_to_rows{1}[0]) {
						push (@tmp_var_true1, $tmp_vname);	
					}
					# if ($row_s == 0) {
					if ($row_s == $map_metal_to_rows{1}[0]) {
						push (@tmp_var_false2, $tmp_vname);
					}
					# elsif ($row_s <= $numTrackH-4) {
					elsif ($row_s <= $map_metal_to_rows{1}[$temp_numTrackH - 4]) {
			...
			if (($dummy_g_struct_flag eq "Split" && $col_idx%2 == 0) || ( $dummy_sd_struct_flag eq "Split" && $col_idx%2 == 1 )) {
				...
				$str.=")) (= 1 1) ";
				# use real col size
				$str.="(ite (bvsge COST_SIZE (_ bv$col ".(length(sprintf("%b", $max_numTrackV))+4).")) ";	
				$str.="(ite (or ";	
				for my $m(0 .. $#tmp_cond_PMOS){
					# 2022/12/29  this was missing
					# exit(-1);
				...
				$str.=") ";
							...
							for my $instIdx (0 .. $lastIdxPMOS) {
								my @tmp_finger = getAvailableNumFinger($inst[$instIdx][2], $trackEachPRow);
								# YW: tmp_finger should use col_idx?
								# keep everythin to index notion except when writing to smt file
								my $Left_x_idx = $col_idx - $tmp_finger[0]*2;
								if ($Left_x_idx < 0) {
									$Left_x_idx = 1;
								}
								for my $Cur_x_idx ($Left_x_idx .. $col_idx) {
									# use real coordinate
									my $Cur_x = $map_metal_to_cols{1}[$Cur_x_idx];
				...
				$CONSTRAINT_ID += 1;
				#$str.=")) ))\n";
			} elsif (($dummy_g_struct_flag eq "Share" && $col_idx%2 == 0))  {
				# Mark: Only for gate
	...
	# 10/26/2022
	$str.=";0.7 Initial Value for contact out of the transistor place location\n";
	for my $i (0 .. $numInstance - 1) {
		my @tmp_finger = ();
		@tmp_finger = getAvailableNumFinger($inst[$i][2], $trackEachPRow);
		#print $out "(assert (= y$i (_ bv".($numPTrackH-$inst[$i][2]/$tmp_finger[0])." ".length(sprintf("%b", $numPTrackH)).")))\n";
		# 1/30: this could be problematic, use index instead
		my $w_idx = (2*$tmp_finger[0]+1);
		# my $real_w = $w * $mapMP{'1'};
		# for my $p_col (1 .. $numTrackV-1) {
		my $temp_numTrackV = $map_numTrackV{1};
		for my $p_col_idx (1 .. $temp_numTrackV-1){
			my $p_col = $map_metal_to_cols{1}[$p_col_idx];
			my @tmp_val_F = ();
			...
			for my $netIndex (0 .. $#nets) {
				for my $vEdgeIndex (0 .. $#virtualEdges) {
					...
					# YW: what about $p_col+$w?
					if (($toCol < $p_col || $toCol >= $map_metal_to_cols{1}[$p_col_idx+$w_idx]) 
						&& $toCol > $map_metal_to_cols_to_idx{1}{0}  
						&& $toCol < $map_metal_to_cols_to_idx{1}{$temp_numTrackV-2} 
						&& $vEdge_Instidx == $i) {
						my $isInNet = 0;
			...
			# Output
			if ($#tmp_val_F > 1) {
				my $length = (length(sprintf("%b", $max_numTrackV))+4);
	...
	# UPDATE FLAG 1
	$str.=";0.8 Only one contact is allowed for internal pin connection\n";
	# Based on the instance location x_i, pin can be found in diff locations
	for my $i (0 .. $numInstance - 1) {
		my @tmp_finger = ();
		@tmp_finger = getAvailableNumFinger($inst[$i][2], $trackEachPRow);
		#print $out "(assert (= y$i (_ bv".($numPTrackH-$inst[$i][2]/$tmp_finger[0])." ".length(sprintf("%b", $numPTrackH)).")))\n";
		# my $w = (2*$tmp_finger[0]+1); # should this be multiplying by M1 MP,no need offset
		# 1/30: this could be problematic, use index instead
		my $w_idx = (2*$tmp_finger[0]+1);
		# my $real_w = $w * $mapMP{'1'};
		# iterate through width index (not real width)
		for my $pin_subidx_idx (1 .. $w_idx) {
			my $pin_subidx = $pin_subidx_idx * $mapMP{"1"};
			#for my $p_col (1 .. $numTrackV-1) {
			my $temp_numTrackV = $map_numTrackV{1};
			for (my $p_col_idx = 1; $p_col_idx < $temp_numTrackV; $p_col_idx = $p_col_idx+2) {
				my $p_col = $map_metal_to_cols{$metal}[$p_col_idx];
				my @tmp_val = ();
				# Metal Virtual Pins
				for my $vEdgeIndex (0 .. $#virtualEdges) {
					my $toRow   = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[2];
					my $toCol   = (split /[a-z]/, $virtualEdges[$vEdgeIndex][1])[3];
					my $len = length(sprintf("%b", $max_numTrackV))+4;
					# if destination is not SON
					if($virtualEdges[$vEdgeIndex][2] ne $keySON){
						# extract instance on destination pin
						my $vEdge_Instidx = $h_inst_idx{$pins[$h_pinId_idx{$virtualEdges[$vEdgeIndex][2]}][6]};	
						# extract destination pin index from pin: pinMM0_6 --> 6
						...
						# if vE origin col >= current gate col
						# && vE origin col <  gate col + instance width 
						# && vE origin col <  number of vertical track - 2 (dummy gates) 
						# && vE Dest pin instance == curr instance
						# && vE Dest pin index == curr pin index
						if (($toCol >= $p_col 
							&& $toCol < $map_metal_to_cols{1}[$p_col_idx+$w_idx]) 
							&& $toCol > $map_metal_to_cols_to_idx{1}{$p_col_idx+$w_idx} 
							&& $toCol < $map_metal_to_cols_to_idx{1}{$temp_numTrackV-2} 
							&& $vEdge_Instidx == $i 
							&& $vEdge_pin_subidx == $pin_subidx_idx) {
							# Metal vE source_ vE dest pin
							my $tmp_vname = "M_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]";
	...
	for my $netIndex (0 .. $#nets) {
		...
		for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
			...
			## Source MaxFlow Indicator
			# NET source pin Instance index
			$instIdx = $h_inst_idx{$pins[$h_pinId_idx{$nets[$netIndex][3]}][6]};
			@tmp_finger = getAvailableNumFinger($inst[$instIdx][2], $trackEachPRow);

			my $temp_numTrackH = $map_numTrackH{1};
			my $temp_numTrackV = $map_numTrackV{1};
			# these are exactly the same because of cfet?
			...
			if($instIdx <= $lastIdxPMOS){ 
				...
				$ru_idx = $h_RTrack{0}; 			# 0 --> 3
				$rl_idx = $h_RTrack{$numPTrackH-1}; # 2-1 = 1 --> 0
				#$r_l = 0;
				#$r_u = $numTrackH/2-2;
				$r_l = 0;
				$r_u = $temp_numTrackH-3;			# 6 - 3 = 3 bc powerrail + 1?
				$c_l = 0;
				$c_u = $temp_numTrackV-1;			#
			}
			else{
				$ru_idx = $h_RTrack{0}; 			# 0 --> 3
				$rl_idx = $h_RTrack{$numPTrackH-1}; # 2-1 = 1 --> 0
				...
				#$r_l = $numTrackH/2-1;
				$r_l = 0;
				$r_u = $temp_numTrackH-3;
				$c_l = 0;
				# $c_u = $numTrackV-1;
				$c_u = $temp_numTrackV-1;
			}
			$ru = $map_metal_to_rows{1}[$ru_idx];
			$rl = $map_metal_to_rows{1}[$rl_idx];
			# iterate through index only
			for my $col_idx ($c_l .. $c_u){
				# source_ofNet pin type is G
				my $col = $map_metal_to_cols{1}[$col_idx];
				if($pins[$h_pinId_idx{$nets[$netIndex][3]}][7] eq "G"){ ### GATE Pin
					my $tmp_pidx = $nets[$netIndex][3];
					# extract pin index (related to transistor width)
					$tmp_pidx =~ s/pin\S+_(\d+)/\1/g;
					$startIdx = 0;
					$endIdx = 0;

					if(($col_idx % 2 == 0)){
						# iterate 0 to last index of tmp_finger
						for my $j(0 .. $#tmp_finger){
							if($j>0) {
								# try different width of tmp_finger?
							...
							if($tmp_pidx >= $startIdx && $tmp_pidx <= $endIdx-1){
								...
								if($tmp_pidx % 2 == 1){
									if($j==0){
										# if($tmp_pidx > $col){
										# comp between pin index and col index
										if($tmp_pidx > $col_idx){
											for my $row_idx ($r_l .. $r_u){
												my $row = $map_metal_to_rows{1}[$row_idx];
												# print("UNSAT BUG: $row");
												# if row is 0, 1, 2, 3
												# if(exists($h_mapTrack{$row_idx}) && $rl <= $row_idx && $ru >= $row_idx){
												if(exists($h_mapTrack{$row_idx}) && $rl_idx <= $row_idx && $ru_idx >= $row_idx){
													# 1/7 bug: only r0 appears here, suppose to have 0-3
													# print("UNSAT BUG: $row\n");
													$vName = "m1r".$row."c".$col;
													$tmp_vname = "N$nets[$netIndex][1]_C$commodityIndex\_E_$vName\_$nets[$netIndex][3]";
#													print $out "(assert (= $tmp_vname false))\n";
											...
											for my $row_idx ($r_l .. $r_u){
												...
												if(exists($h_mapTrack{$row_idx}) && $rl_idx <= $row_idx && $ru_idx >= $row_idx){
													# 1/7 bug: only r0 appears here, suppose to have 0-3
													# print("UNSAT BUG: $row, $rl_idx, $ru_idx\n");
													if(!exists($h_assign{$tmp_vname})){
											...
											if($cnt_true>1){
												...
												# this should be real col as it represents instance starting location
												$str.=" (= x$instIdx (_ bv".($map_metal_to_cols{1}[$col_idx-$tmp_pidx])." $len))";
												cnt("l", 1);
												if($cnt_true==1){
													$str.=" (and";
													for my $m(0 .. $#tmp_var){
								...
								if($tmp_pidx % 4 == 0){
									if($j==0){
										...
										if($tmp_pidx <= $col_idx){
											...
											# this should be real col as it represents instance starting location
											$tmp_str_1.=" (= x$instIdx (_ bv".$map_metal_to_cols{1}[$col_idx-$tmp_pidx]." $len)))";
											cnt("l", 1);
										}
									}
									last;
							...
							if($tmp_pidx >= $startIdx && $tmp_pidx <= $endIdx-1){
								if($tmp_pidx % 4 == 0){
									if($j==0){
										...
										if (((($startIdx + $endIdx - 1 - $tmp_pidx)%2==0?($startIdx + $endIdx - 1 - $tmp_pidx):($startIdx + $endIdx - 1 - $tmp_pidx + 1))) <= $col_idx){
											if($isValid == 0){
											...
											$tmp_str_1.=" (= x$instIdx (_ bv".$map_metal_to_cols{1}[($col_idx-((($startIdx + $endIdx - 1 - $tmp_pidx)%2==0?($startIdx + $endIdx - 1 - $tmp_pidx):($startIdx + $endIdx - 1 - $tmp_pidx + 1))))]." $len)))";
											cnt("l", 1);
										}
									}
									last;
...
#									print $out "(assert (= $tmp_vname false))\n";
			...
			if($instIdx <= $lastIdxPMOS){ 
				...
				# $rl = $h_RTrack{$numPTrackH-1};
				#$r_u = $numTrackH/2-2;
				$ru_idx = $h_RTrack{0};				# upper --> 3
				$rl_idx = $h_RTrack{$numPTrackH-1};	# lower --> 0
				$r_l = 0;
				$r_u = $temp_numTrackH-3;			# 6 - 3 = 3 bc powerrail + 1?
				$c_l = 0;
				$c_u = $temp_numTrackV-1;			# 
...
#									print $out "(assert (= $tmp_vname false))\n";
	...
	if($Local_Parameter == 1){
		$str.=";0.9 Localization: Conditional Localization for All Commodities\n\n";
		for my $netIndex (0 .. $#nets) {
			for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
				my $inst_pin_s = $h_inst_idx{$pins[$h_pinId_idx{$nets[$netIndex][3]}][6]};
				my $inst_pin_t = $h_inst_idx{$pins[$h_pinId_idx{$nets[$netIndex][5][$commodityIndex]}][6]};
				# 1/22 disable for now based on probe3
				...
				my %h_edge = ();
				if($nets[$netIndex][5][$commodityIndex] ne $keySON){
					# 1/22 starts all commodity w.r.t M1 only
					for my $col (@{$map_metal_to_cols{1}}){
						# translate to m1 idx
						my $m1_col_idx = $map_metal_to_cols_to_idx{1}{$col};
						...
						cnt("l", 1);
						cnt("l", 1);
						$str.="             (and (ite (bvslt x$inst_pin_s (_ bv".$map_metal_to_cols{1}[($m1_col_idx-$tolerance-$pidx_s>=0?($m1_col_idx-$tolerance-$pidx_s):(0))]." $len)) (and"; 
						cnt("l", 1);

						# 1/22 extract all rows to iterate on (excluding pwr rail)
						# 1
						my @allRows = sort { $a <=> $b } uniq(@{$map_metal_to_rows{2}}, @{$map_metal_to_rows{4}});
						for my $row (@allRows) {
							for my $metal (1 .. $numMetalLayer) {
								# unless col is in this metal and row is in this metal
								unless ($col ~~ @{$map_metal_to_cols{$metal}} && $row ~~ @{$map_metal_to_rows{$metal}}) {
									next;
								}
								# 1/22 disable col % 2 = 1 bc this is not on M1
						...
						$str.=") (= true true))";
						$str.="                  (ite (bvsgt x$inst_pin_t (_ bv".$map_metal_to_cols{1}[($m1_col_idx+$tolerance-$pidx_t>=$map_numTrackV{1}-1?($map_numTrackV{1}-1):($m1_col_idx+$tolerance-$pidx_t>=0?($m1_col_idx+$tolerance-$pidx_t):(0)))]." $len)) (and"; 
						cnt("l", 1);
						%h_edge = ();
						# 2
						for my $row (@allRows) {
						...
						$str.=") (= true true)))";
						$str.="             (and (ite (bvslt x$inst_pin_t (_ bv".$map_metal_to_cols{1}[($m1_col_idx-$tolerance-$pidx_t>=0?($m1_col_idx-$tolerance-$pidx_t):(0))]." $len)) (and"; 
						cnt("l", 1);
						%h_edge = ();
						# 3
						for my $row (@allRows) {
						...
						$str.=") (= true true))";
						$str.="                  (ite (bvsgt x$inst_pin_s (_ bv".$map_metal_to_cols{1}[($m1_col_idx+$tolerance-$pidx_s>=$map_numTrackV{1}-1?($map_numTrackV{1}-1):($m1_col_idx+$tolerance-$pidx_s>=0?($m1_col_idx+$tolerance-$pidx_s):(0)))]." $len)) (and"; 
						cnt("l", 1);
						%h_edge = ();
						# 4
						for my $row (@allRows) {
						...
						$str.=") (= true true))";
						$str.="                  (ite (bvsgt x$inst_pin_t (_ bv".$map_metal_to_cols{1}[($m1_col_idx+$tolerance-$w_t+$pidx_t>=$map_numTrackV{1}-1?($map_numTrackV{1}-1):($m1_col_idx+$tolerance-$w_t+$pidx_t>=0?($m1_col_idx+$tolerance-$w_t+$pidx_t):(0)))]." $len)) (and"; 
						cnt("l", 1);
						%h_edge = ();
						# 6
						for my $row (@allRows) {
						...
						$str.=") (= true true)))";
						$str.="             (and (ite (bvslt x$inst_pin_t (_ bv".$map_metal_to_cols{1}[($m1_col_idx-$tolerance-$w_t+$pidx_t>=0?($m1_col_idx-$tolerance-$w_t+$pidx_t):(0))]." $len)) (and"; 
						cnt("l", 1);
						%h_edge = ();
						# 7
						for my $row (@allRows) {
						...
						cnt("l", 1);
						cnt("l", 1);
						$str.="             (and (ite (bvslt x$inst_pin_s (_ bv".$map_metal_to_cols{1}[($m1_col_idx-$tolerance-$w_s+$pidx_s>=0?($m1_col_idx-$tolerance-$w_s+$pidx_s):(0))]." $len)) (and"; 
						cnt("l", 1);
						%h_edge = ();
						# 9
						for my $row (@allRows) {
						...
						$str.=") (= true true))";
						$str.="                  (ite (bvsgt x$inst_pin_s (_ bv".$map_metal_to_cols{1}[($m1_col_idx+$tolerance-$w_s+$pidx_s>=$map_numTrackV{1}-1?($map_numTrackV{1}-1):($m1_col_idx+$tolerance-$w_s+$pidx_s>=0?($m1_col_idx+$tolerance-$w_s+$pidx_s):(0)))]." $len)) (and"; 
						cnt("l", 1);
						%h_edge = ();
						# 12
						for my $row (@allRows) {
						...
						$str.=") (= true true))))))):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
						cnt("c", 1);
					}
				}
					...
					for my $col_idx (0 .. $temp_numTrackV-1) {
						...
						# true-assigned variable is not included in terms
							...
							if($cnt_var == 1){
								...
								$str.="(assert (!(= (or (not $tmp_var[0]) $tmp_var[1]) true):named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("l", 1);
								cnt("l", 1);
								cnt("c", 1);
								$str.="(assert (!(= (or $tmp_var[0] (not $tmp_var[1])) true):named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("l", 1);
								cnt("l", 1);
								cnt("c", 1);
								...
								for my $i(0 .. $#tmp_var){
									...
									$str.=") true):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("c", 1);
								}
							}
...
# =begin
	...
	for my $netIndex (0 .. $#nets) {
		for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
			...
			for my $pinIndex (0 .. $#pins) {
				# 1/7 vname from pins, this should be on M1
					...
					if($vName eq $keySON){
						if($EXT_Parameter == 0){
							...
							for my $i (0 .. $#{$vedge_in{$vName}}){ # sink
								...
								# print("whats this? $map_metal_to_rows{$metal}[-1]\n");
								if($metal%2 == 1 && ($row == $map_metal_to_rows{$metal}[0] || $row == $map_metal_to_rows{$metal}[-1])){
									$tmp_str ="N$nets[$netIndex][1]\_";
									$tmp_str.="C$commodityIndex\_";
						...
						## Sink MaxFlow Indicator
						$instIdx = $h_inst_idx{$pins[$h_pinId_idx{$nets[$netIndex][5][$commodityIndex]}][6]};
						# convert to idx and then real row
						my $temp_numTrackH = $map_numTrackH{1};
						if($instIdx <= $lastIdxPMOS){ 
							#potential bug
							# $r_l = 0;
		...
		for my $udeIndex (0 .. $#udEdges) {
			for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
				...
				if(!exists($h_assign{$tmp_com}) && !exists($h_assign{$tmp_net})){
					setVar($tmp_com, 2);
					setVar($tmp_net, 2);
					$str.="(assert (! (ite (= $tmp_com true) (= $tmp_net true) (= 1 1)) :named c$CONSTRAINT_ID))\n";
					$CONSTRAINT_ID += 1;
					cnt("l", 1);
					cnt("l", 1);
					cnt("c", 1);
			...
			if ($virtualEdges[$vEdgeIndex][2] =~ /^pin/) { # source
				...
				if($isInNet == 1){
					...
					if ($#tmp_com_arr+1 > 0) {
						...
						$str.=") (= $tmp_net false) (= 1 1)):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
						cnt("c", 1);
					}
				}
	...
	for my $vEdgeIndex (0 .. $#virtualEdges) {
		...
		if($cnt_true>0){
			...
			if(exists($h_assign{$tmp_str_metal}) && $h_assign{$tmp_str_metal} == 0){
				...
				setVar($tmp_str_metal, 2);
				$str.="(assert (!(= $tmp_var[0] $tmp_str_metal):named c$CONSTRAINT_ID))\n";
				$CONSTRAINT_ID += 1;
				cnt("l", 1);
				cnt("l", 1);
				cnt("c", 1);
			...
			for my $row_idx (0 .. $temp_numTrackH-3){
				...
				for my $col_idx (0 .. $temp_numTrackV-1){
					...
					if(!exists($h_assign{$tmp_g}) && !exists($h_assign{$tmp_m1})){
						setVar($tmp_g, 6);
						setVar($tmp_m1, 2);
						$str.="(assert (! ((_ at-most 1) $tmp_g $tmp_m1):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
						cnt("l", 3);
						cnt("l", 3);
						cnt("c", 3);
					...
					if(!exists($h_assign{$tmp_g}) && !exists($h_assign{$tmp_m2})){
						setVar($tmp_g, 6);
						setVar($tmp_m2, 2);
						$str.="(assert (! (ite (= $tmp_g true) (= $tmp_m2 true) (= true true)):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
						cnt("l", 3);
						cnt("l", 3);
						cnt("c", 3);
					...
					if(!exists($h_assign{$tmp_g}) && !exists($h_assign{$tmp_m1})){
						if(!exists($h_assign{$tmp_m2})){
							...
							setVar($tmp_m1, 2);
							setVar($tmp_m2, 2);
							$str.="(assert (! (ite (= (or $tmp_g $tmp_m1) false) (= $tmp_m2 false) (= true true)):named c$CONSTRAINT_ID))\n";
							$CONSTRAINT_ID += 1;
							cnt("l", 3);
							cnt("l", 3);
							cnt("l", 3);
						...
						if(exists($h_assign{$tmp_m1}) && $h_assign{$tmp_m1} == 0){
							...
							if(!exists($h_assign{$tmp_m2})){
								setVar($tmp_m1, 2);
								setVar($tmp_m2, 2);
								$str.="(assert (! (ite (= $tmp_m1 false) (= $tmp_m2 false) (= true true)) :named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("l", 3);
								cnt("l", 3);
								cnt("c", 3);
						...
						if(exists($h_assign{$tmp_g}) && $h_assign{$tmp_g} == 0){
							...
							if(!exists($h_assign{$tmp_m2})){
								setVar($tmp_g, 2);
								setVar($tmp_m2, 2);
								$str.="(assert (! (ite (= $tmp_g false) (= $tmp_m2 false) (= true true)) :named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("l", 3);
								cnt("l", 3);
								cnt("c", 3);
			...
			for my $row_idx (0 .. $temp_numTrackH-3){
				...
				for my $col_idx (0 .. $temp_numTrackV-1){
					...
					if(!exists($h_assign{$tmp_g}) && !exists($h_assign{$tmp_m1})){
						setVar($tmp_g, 6);
						setVar($tmp_m1, 2);
						$str.="(assert (! (ite (= $tmp_g true) (= $tmp_m1 true) (= true true)):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
						cnt("l", 3);
						cnt("l", 3);
						cnt("c", 3);
					...
					if(!exists($h_assign{$tmp_g}) && !exists($h_assign{$tmp_m2})){
						setVar($tmp_g, 6);
						setVar($tmp_m2, 2);
						$str.="(assert (! ((_ at-most 1) $tmp_g $tmp_m2):named c$CONSTRAINT_ID))\n";
						$CONSTRAINT_ID += 1;
						cnt("l", 3);
						cnt("l", 3);
						cnt("c", 3);
					...
					if(!exists($h_assign{$tmp_g}) && !exists($h_assign{$tmp_m2})){
						if(!exists($h_assign{$tmp_m1})){
							...
							setVar($tmp_m2, 2);
							setVar($tmp_m1, 2);
							$str.="(assert (! (ite (= (or $tmp_g $tmp_m2) false) (= $tmp_m1 false) (= true true)):named c$CONSTRAINT_ID))\n";
							$CONSTRAINT_ID += 1;
							cnt("l", 3);
							cnt("l", 3);
							cnt("l", 3);
						...
						if(exists($h_assign{$tmp_m2}) && $h_assign{$tmp_m2} == 0){
							...
							if(!exists($h_assign{$tmp_m1})){
								setVar($tmp_m2, 2);
								setVar($tmp_m1, 2);
								$str.="(assert (! (ite (= $tmp_m2 false) (= $tmp_m1 false) (= true true)):named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("l", 3);
								cnt("l", 3);
								cnt("c", 3);
						...
						if(exists($h_assign{$tmp_g}) && $h_assign{$tmp_g} == 0){
							...
							if(!exists($h_assign{$tmp_m1})){
								setVar($tmp_g, 2);
								setVar($tmp_m1, 2);
								$str.="(assert (!(ite (= $tmp_g false) (= $tmp_m1 false) (= true true)):named c$CONSTRAINT_ID))\n";
								$CONSTRAINT_ID += 1;
								cnt("l", 3);
								cnt("l", 3);
								cnt("c", 3);
		...
		for my $metal (2 .. $numMetalLayer) { # no DR on M1 
			if ($metal % 2 == 0) {  # M2/M4
				...
				for my $row_idx (0 .. $temp_numTrackH-3){
					my $row = $map_metal_to_rows{$metal}[$row_idx];
					my $temp_numTrackV = $map_numTrackV{$metal};
					for my $col_idx (0 .. $temp_numTrackV - $MAR_Parameter) {
						my $col = $map_metal_to_cols{$metal}[$col_idx];
						my $tmp_str="";
						my @tmp_var = ();
						my $cnt_var = 0;
						...
						for my $marIndex (0 .. $MAR_Parameter-1){
							my $colNumber_idx = $col_idx + $marIndex;
							my $varName = "m".$metal."r".$row."c".$map_metal_to_cols{$metal}[$colNumber_idx];
				...
				my $temp_numTrackH = $map_numTrackH{$metal};
				for my $row_idx (0 .. $temp_numTrackH - 3 - $MaxIndex){
					my $row = $map_metal_to_rows{$metal}[$row_idx];
					my $temp_numTrackV = $map_numTrackV{$metal};
					for my $col_idx (0 .. $temp_numTrackV-1) {
						my $col = $map_metal_to_cols{$metal}[$col_idx];
						...
						my $newRow_idx = $row_idx;

						if( ($vertices{$varName}[5][3] eq "null") || (( $DoublePowerRail == 1) && (($newRow_idx + 1) % ($trackEachRow + 1) == 0) && ($MAR_Parameter == 2)) ){
						...
						for my $marIndex (0 .. $MAR_Parameter-1){
							...
							if ( ($DoublePowerRail == 1) && ($newRow_idx % ($trackEachRow + 1) == 0) && ($powerRailFlag < $powerRailIndex) ){
								$powerRailFlag++;
								$newRow_idx++;
								next;
							}
		...
		for my $metal (2 .. $numMetalLayer) { # no DR on M1
			if ($metal % 2 == 0) {  # M2
				...
				for my $row_idx (0 .. $temp_numTrackH-3){
					...
					# YW: not skipping anymore?
					if ($row_idx == 0 || $row_idx == ($temp_numTrackH-3)){
#                    next;
					}
					...
					for my $col_idx (0 .. $temp_numTrackV-2){
						...
						# 1/30 bug found: not using numTrack correctly
						if ( ($vName_BR ne "null") && ($row != $temp_numTrackH - 3) && ($EOL_Parameter >= 2)) {
							my $tmp_str="";
							my @tmp_var = ();
							my $cnt_var = 0;
							my $cnt_true = 0;
		...
		for my $metal (2 .. $numMetalLayer) { # no DR on M1
			if ($metal % 2 == 0) {
        		...
        		for my $row_idx (0 .. $temp_numTrackH-3){
					...
					# 	$vName = "m".$metal."r".$row."c".$col;
					my $temp_numTrackV = $map_numTrackV{$metal};
					for my $col_idx (1 .. $temp_numTrackV-1){
							my $col = $map_metal_to_cols{$metal}[$col_idx];
							$vName = "m".$metal."r".$row."c".$col;

						# FL Direction Checking
						my $vName_FL = $vertices{$vName}[5][6];
						if (($vName_FL ne "null") && ($row_idx != 0) && ($EOL_Parameter >= 2)) {
							my $tmp_str="";
							my @tmp_var = ();
							my $cnt_var = 0;
							my $cnt_true = 0;
						...
						if ( ($vName_BL ne "null") && ($row_idx != $temp_numTrackH - 3) && ($EOL_Parameter >= 2)) {
							my $tmp_str="";
							my @tmp_var = ();
							my $cnt_var = 0;
							my $cnt_true = 0;
				...
				# for my $row (0 .. $numTrackH-4) {
					...
					# for my $col (0 .. $numTrackV-1) {
						...
						if (($metal >= 2 && $metal % 2 == 0) && ($vName_BL ne "null") && ($EOL_Parameter >= 2)) {
							# my $newRow = $row + 1;
							my $newRow_idx = $row_idx + 1;
							if ( ($DoublePowerRail == 1) && ($newRow_idx % ($trackEachRow + 1) == 0) && ($EOL_Parameter == 2)){ # Skip the BR Direction
						...
						if ($vName_B ne "null") {
							# my $newRow = $row + 1;
							my $newRow_idx = $row_idx + 1;
							if (($DoublePowerRail == 1) && ($newRow_idx % ($trackEachRow + 1) == 0) && ($EOL_Parameter == 1)){
						...
						if (($metal>=2 && $metal % 2 == 0) && ($vName_BR ne "null") && ($EOL_Parameter >= 2)) {
							# my $newRow = $row + 1;
							my $newRow_idx = $row_idx + 1;
							if (($DoublePowerRail == 1) && ($newRow_idx % ($trackEachRow + 1) == 0) && ($EOL_Parameter == 2)){ # Skip the BL Direction
		...
		for my $metal (1 .. $numMetalLayer) { # no DR on M1
			if ($metal % 2 == 1) {
				...
				my $temp_numTrackH = $map_numTrackH{$metal};
				for my $row_idx (1 .. $temp_numTrackH-3){
					my $row = $map_metal_to_rows{$metal}[$row_idx];	
					# for my $col (0 .. $numTrackV-1) {
					my $temp_numTrackV = $map_numTrackV{$metal};
					for my $col_idx (0 .. $temp_numTrackV-1){
						...
						if ($vName_F ne "null") {
							...
							my $newRow_idx = $row_idx - 1;
							# if (($DoublePowerRail == 1) && ($newRow % ($trackEachRow + 1) == 0) && ($EOL_Parameter == 1)){ # Skip the B Direction
							if (($DoublePowerRail == 1) && ($newRow_idx % ($trackEachRow + 1) == 0) && ($EOL_Parameter == 1)){ # Skip the B Direction
								### Nothing
							}
							else{
								my $tmp_str="";
	...
	if ( $VR_Parameter == 0 ){
		...
		#for my $metal (1 .. 1) { # no DR on M1
			...
			# YW: VR can double stack if same net
			...
			# for my $row (0 .. $numTrackH-3) {
				...
				for my $col_idx (0 .. $temp_numTrackV-1){
					my $col = $map_metal_to_cols{$metal}[$col_idx];
					# skip if via not constructable
					unless ($metal != $numMetalLayer && $row ~~ $map_metal_to_rows{$metal+1} && $col ~~ $map_metal_to_cols{$metal+1}){
						next;
					}
				# for my $col (0 .. $numTrackV-1) {
					# 1/22 disable odd col
					...
					# 	next;
					# }
					if (($row_idx == $temp_numTrackH-3) && ($col_idx == $temp_numTrackV-1)) {
					...
					if ($vertices{$vName}[5][4] ne "null") { # Up Neighbor, i.e., VIA from the vName vertex.
						...
						for my $newRow_idx ($row_idx .. $temp_numTrackH-3) {
							# for my $newCol ($col .. $numTrackV-1) {
							for my $newCol_idx ($col_idx .. $temp_numTrackV-1) {
								# my $distCol = $newCol - $col;
								# my $distRow = $newRow - $row;
								my $distCol_idx = $newCol_idx - $col_idx;
								my $distRow_idx = $newRow_idx - $row_idx;

								# Check power rail between newRow and row. (Double Power Rail Rule Applying
								if ( ($DoublePowerRail == 1) && (floor($newRow_idx / ($trackEachRow + 1)) ne floor($row_idx / ($trackEachRow + 1))) ){
								# if ( ($DoublePowerRail == 1) && (floor($newRow / ($trackEachRow + 1)) ne floor($row / ($trackEachRow + 1))) ){
									# $distRow++;
									$distRow_idx++;
								}
								# if ( ($newRow eq $row) && ($newCol eq $col) ){  # Initial Value.
								if ( ($newRow_idx eq $row_idx) && ($newCol_idx eq $col_idx) ){  # Initial Value.
									next;
								}
								# if ( ($distCol * $distCol + $distRow * $distRow) > ($VR_Parameter * $VR_Parameter) ){ # Check the Via Distance
								if ( ($distCol_idx * $distCol_idx + $distRow_idx * $distRow_idx) > ($VR_Parameter * $VR_Parameter) ){ # Check the Via Distance
									last;
								}
								...
								while ($distRow_idx > 0){  
									...
									if( ($DoublePowerRail == 1) && ($currentRow_idx % ($trackEachRow + 1) == 0) && ($FlagforSecond == 0) ){ #power Rail
										$FlagforSecond = 1;
										# $currentRow--;
										$currentRow_idx--;
										next;
								...
								if ($neighborName ne "null"){
									...
									if ($neighborUp eq "null"){
										next; # 1/20 disable for now
										print "ERROR(a) : There is some bug in switch box definition !\n";
										print "$vName\n";
										exit(-1);
									}
								}
								# re-extracted neighbor's real col ==> col_idx
								my $col_neighbor = (split /[mrc]/, $neighborName)[3];
								my $col_neighbor_idx = $map_metal_to_cols_to_idx{$metal}{$col_neighbor};

								# if($metal > 1 && $metal % 2 == 1 && ($col_neighbor %2 == 1)){
								if($metal > 1 && $metal % 2 == 1 && ($col_neighbor_idx % 2 == 1)){
									next;
								}
								else{
							...
							for my $newRow_idx ($row_idx .. $temp_numTrackH-3) {
								...
								for my $newCol_idx ($col_idx .. $temp_numTrackV-1) {
									...
									if ($neighborName ne "null"){
										...
										if ($neighborUp eq "null"){
											next; # 1/20 disable for now
											print "ERROR(b) : There is some bug in switch box definition !\n";
											print "$vName, $neighborName\n";
											exit(-1);
										}
									}
									my $col_neighbor = (split /[mrc]/, $neighborName)[3];
						...
						for my $newRow_idx ($row_idx .. $temp_numTrackH-3) {
							...
							for my $newCol_idx ($col_idx .. $temp_numTrackV-1) {
								...
								if ($neighborName ne "null"){
									...
									if ($neighborUp eq "null"){
										next; # 1/20 disable for now
										print "ERROR(d) : There is some bug in switch box definition !\n";
										print "$vName\n";
										exit(-1);
									}
								}
		...
		} # End $VR_double_samenet_flag == 1 # 1/22 New to CFET
		...
		for my $metal (2 .. $numMetalLayer - 1) { # no DR on M1
			...
			for my $row_idx (0 .. $temp_numTrackH-3){
				...
				for my $col_idx (0 .. $temp_numTrackV-1){
					...
					# skip if stacked via not constructable
					unless ($metal != $numMetalLayer 
						&& $row ~~ $map_metal_to_rows{$metal+1} && $col ~~ $map_metal_to_cols{$metal+1} 
						&& $row ~~ $map_metal_to_rows{$metal-1} && $col ~~ $map_metal_to_cols{$metal-1}) {
						next;
					}
					...
					if ( ($vertices{$vName}[5][4] eq "null") || ($vertices{$vName}[5][5] eq "null") ){
						print "ERROR(c) : There is some bug in switch box definition !\n";
						print "$vertices{$vName}[1] $vertices{$vName}[5][5] $vertices{$vName}[5][4]\n";
						exit(-1);
					}
					my $tmp_str="";
					my @tmp_var = ();
					...
					# print "c30 $vertices{$vName}[1], $vertices{$vName}[5][5]\n";
			...
			if ($metal % 2 == 0) {  # M2
				...
				for my $row_idx (0 .. $temp_numTrackH - 3){
					...
					for my $col_idx (0 .. $temp_numTrackV-1) {
						...
						if ( ($vName_B ne "null") && ($row_idx != $temp_numTrackH - 3)) {
							my $tmp_str="";
							my @tmp_var = ();
							my $cnt_var = 0;
							my $cnt_true = 0;
			...
			if ($metal % 2 == 0) {  # M2
				...
				for my $row_idx (0 .. $temp_numTrackH - 3){
					...
					for my $col_idx (0 .. $temp_numTrackV-1) {
						...
						if ( ($vName_FR ne "null") && ($row_idx != 0) && ($PRL_Parameter > 1) ) {
							my $tmp_str="";
							my @tmp_var = ();
							my $cnt_var = 0;
							my $cnt_true = 0;
						...
						if ( ($vName_BR ne "null") && ($row_idx != $temp_numTrackH - 3) && ($PRL_Parameter > 1) ) {
							my $tmp_str="";
							my @tmp_var = ();
							my $cnt_var = 0;
							my $cnt_true = 0;
						...
						if ($vName_L ne "null") {
							...
							if ($PRL_Parameter > 1){
								...
								for my $prlIndex (0 .. $PRL_Parameter-2){
									if (($DoublePowerRail == 1) && ($newRow_idx % ($trackEachRow + 1) == 0) && ($FlagforSecond == 0)){
										$FlagforSecond = 1;
										next;
									}   
									$FlagforSecond = 0;
				...
				for my $row_idx (0 .. $temp_numTrackH-4) {
					...
					for my $col_idx (0 .. $temp_numTrackV-1) {
						...
						if (($vName_L ne "null") && ($PRL_Parameter > 1)) { 
							my $newRow_idx = $row_idx+1;
							if( ($DoublePowerRail == 1) && ($newRow_idx % ($trackEachRow + 1) == 0) && ($PRL_Parameter == 2)) {
								#### Nothing
							}
							else{
								my $tmp_str="";
				...
				for my $col_idx (0 .. $temp_numTrackV-1){
					...
					if(!exists($h_tmp{$net."_".$col})){
						...
						for my $row_idx (0 .. $temp_numTrackH - 3){
							...
							$vName = "m".$metal."r".$row."c".$col; # incoming
							for my $i (0 .. $#{$vedge_out{$vName}}) {
								for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
									if($virtualEdges[$vedge_out{$vName}[$i]][2] eq $nets[$netIndex][5][$commodityIndex] && $virtualEdges[$vedge_out{$vName}[$i]][2] eq $keySON) {
										...
										if(!exists($h_assign{$tmp_str_e}) || exists($h_assign{$tmp_str_e}) && $h_assign{$tmp_str_e} eq 1){
											...
											for my $row2_idx (0 .. $temp_numTrackH-3) {
												my $row2 = $map_metal_to_rows{$metal}[$row2_idx];
												...
												if(exists($vertices{$vName2}) && $vertices{$vName2}[5][3] ne "null"){
													...
													my $cnt_var_com = 0;
													my $cnt_true_com = 0;
													# YW: find real row2 ==> vName3
													# my $vName3 = "m".$metal."r".($row2+1)."c".$col;
													my $vName3 = "m".$metal."r".$map_metal_to_rows{$metal}[$row2_idx+1]."c".$col;
													...
													for my $row3_idx ($row2_idx+1 .. $temp_numTrackH-3) {
														my @tmp_var_net = ();
														my @tmp_var_com = ();
														my $cnt_var_net = 0;
														my $cnt_var_com = 0;
														...
														for my $j (0 .. $row3_idx-$row2_idx-1){
															# my $vName2 = "m".$metal."r".($row2+1+$j)."c".($col);
															my $vName2 = "m".$metal."r".($map_metal_to_rows{$metal}[$row2_idx+1+$j])."c".($col);
															if(exists($vertices{$vName2}) && $vertices{$vName2}[5][3] ne "null"){
																$tmp_str = "N$nets[$netIndex][1]\_E_$vName2\_$vertices{$vName2}[5][3]";
																if(!exists($h_assign{$tmp_str})) {
														...
														my $vName3 = "m".$metal."r".($map_metal_to_rows{$metal}[$row3_idx+1])."c".($col);
														if(exists($vertices{$vName3}) && $vertices{$vName3}[5][3] ne "null"){
															for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
																$tmp_str = "N$nets[$netIndex][1]\_C$commodityIndex\_E_$vName3\_$vertices{$vName3}[5][3]";
																if(!exists($h_assign{$tmp_str})){
													...
													my $vName3 = "m".$metal."r".$map_metal_to_rows{$metal}[$row2_idx]."c".($col);
													...
													for my $row3_idx (0 .. $row2_idx-1){
														...
														# for my $j (0 .. $row3){
														for my $j (0 .. $row3_idx){
															my $vName2 = "m".$metal."r".($map_metal_to_rows{$metal}[$row2_idx-$j])."c".($col);
														...
														my $vName3 = "m".$metal."r".($map_metal_to_rows{$metal}[$row2_idx-$row3_idx-1])."c".($col);
														if(exists($vertices{$vName3}) && $vertices{$vName3}[5][2] ne "null"){
															for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
																$tmp_str = "N$nets[$netIndex][1]\_C$commodityIndex\_E_$vertices{$vName3}[5][2]\_$vName3";
																if(!exists($h_assign{$tmp_str})){
...
### DATA STRUCTURE:  ADJACENT_VERTICES [0:Left] [1:Right] [2:Front] [3:Back] [4:Up] [5:Down] [6:FL] [7:FR] [8:BL] [9:BR]
	...
	# 12/7/2022 UPDATE FLAG 2
	$str.=";13-C. VIA23 for each vertex\n";
	foreach my $net(keys %h_extnets){
		my $netIndex = $h_idx{$net};
		# YW: this is top-1 metal layer
						...
						if(!exists($h_assign{$tmp1}) && !exists($h_assign{$tmp2})){
							if(exists($h_assign{$tmp3}) && $h_assign{$tmp3} == 0){
								if(exists($h_assign{$tmp4}) && $h_assign{$tmp4} == 0){
									setVar($tmp1, 6);
									setVar($tmp2, 6);
									$str.="(assert (!(ite (and (= $tmp1 true) (= $tmp2 true)) (= true false) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("c", 3);
									...
									setVar($tmp2, 6);
									setVar($tmp4, 6);
									$str.="(assert (!(ite (and (= $tmp1 true) (= $tmp2 true)) (= $tmp4 true) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("l", 3);
								...
								if(exists($h_assign{$tmp4}) && $h_assign{$tmp4} == 0){
									...
									setVar($tmp2, 6);
									setVar($tmp3, 6);
									$str.="(assert (!(ite (and (= $tmp1 true) (= $tmp2 true)) (= $tmp3 true) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("l", 3);
									...
									setVar($tmp3, 6);
									setVar($tmp4, 6);
									$str.="(assert (!(ite (and (= $tmp1 true) (= $tmp2 true)) ((_ at-least 1) $tmp3 $tmp4) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("l", 3);
							...
							if(exists($h_assign{$tmp3}) && $h_assign{$tmp3} == 0){
								if(exists($h_assign{$tmp4}) && $h_assign{$tmp4} == 0){
									setVar($tmp2, 6);
									$str.="(assert (!(ite (= $tmp2 true) (= true false) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("c", 3);
								}
									...
									setVar($tmp2, 6);
									setVar($tmp4, 6);
									$str.="(assert (!(ite (= $tmp2 true) (= $tmp4 true) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("c", 3);
								...
								if(exists($h_assign{$tmp4}) && $h_assign{$tmp4} == 0){
									setVar($tmp2, 6);
									setVar($tmp3, 6);
									$str.="(assert (!(ite (= $tmp2 true) (= $tmp3 true) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("c", 3);
									...
									setVar($tmp3, 6);
									setVar($tmp4, 6);
									$str.="(assert (!(ite (= $tmp2 true) ((_ at-least 1) $tmp3 $tmp4) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("l", 3);
							...
							if(exists($h_assign{$tmp3}) && $h_assign{$tmp3} == 0){
								if(exists($h_assign{$tmp4}) && $h_assign{$tmp4} == 0){
									setVar($tmp1, 6);
									$str.="(assert (! (ite (= $tmp1 true) (= true false) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("c", 3);
								}
									...
									setVar($tmp1, 6);
									setVar($tmp4, 6);
									$str.="(assert (! (ite (= $tmp1 true) (= $tmp4 true) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("c", 3);
								...
								if(exists($h_assign{$tmp4}) && $h_assign{$tmp4} == 0){
									setVar($tmp1, 6);
									setVar($tmp3, 6);
									$str.="(assert (! (ite (= $tmp1 true) (= $tmp3 true) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("c", 3);
									...
									setVar($tmp3, 6);
									setVar($tmp4, 6);
									$str.="(assert (! (ite (= $tmp1 true) ((_ at-least 1) $tmp3 $tmp4) (= true true)):named c$CONSTRAINT_ID))\n";
									$CONSTRAINT_ID += 1;
									cnt("l", 3);
									cnt("l", 3);
									cnt("l", 3);
...
foreach my $key(keys %h_assign){
	if($h_assign{$key} == 1){
		print $out "(assert (!(= $key true):named c$CONSTRAINT_ID))\n";
		$CONSTRAINT_ID += 1;
	}
}
	...
	foreach my $key(keys %NMOS_forbidden){
		if ($key =~ m/^M/ ) {
			if($NMOS_forbidden{$key} == 0){
				print $out "(assert (!(= $key false):named c$CONSTRAINT_ID))\n";
				$CONSTRAINT_ID += 1;
			}
		}
	}
...
# NOTE: block the rest for debugging purpose
# 12/7/2022 UPDATE FLAG 4
...
print $out "(assert (bvsle COST_SIZE_P (_ bv$max_numTrackV ".(length(sprintf("%b", $max_numTrackV))+4).")))\n";
print $out "(assert (bvsle COST_SIZE_N (_ bv$max_numTrackV ".(length(sprintf("%b", $max_numTrackV))+4).")))\n";

# YW: Switch order of iteration so we can call metal specific row/col
# for my $udeIndex (0 .. $#udEdges) {
# 	print("uwu: $udEdges[$udeIndex][1]\n");
# 	my $fromMetal = (split /[a-z]/, $udEdges[$udeIndex][1])[1]; # 1:metal 2:row 3:col
...
# 	# for my $row (0 .. $numTrackH -3){
# 	my $temp_numTrackH = $map_numTrackH{$toMetal};
# 	for my $row_idx (0 .. $temp_numTrackH-3) {
# 		my $row = $map_metal_to_rows{$toMetal}[$row_idx];
# 		print $out "(assert (= M2_TRACK_$row (or";

# 		my $temp_numTrackV = $map_numTrackV{$toMetal};
# 		for my $col_idx (0 .. $temp_numTrackV-1) {
# 			my $col = $map_metal_to_cols{$toMetal}[$col_idx];
...
# 	}
# }
# iterate on all possible row/col
my $iffirst = 0;
my $ifwritten = 0;
# 1/22 iterate through all possible rows
my @allrows = sort { $a <=> $b } uniq(@{$map_metal_to_rows{2}}, @{$map_metal_to_rows{4}});
for my $row (@allrows){
	$iffirst = 1;
	# print $out "(assert (= M2_TRACK_$row (or";
	# 1/22 iterate through all possible cols
	my @allcols = sort { $a <=> $b } uniq(@{$map_metal_to_cols{1}}, @{$map_metal_to_cols{3}});
	for my $col (@allcols){
		my $len = length(sprintf("%b", $max_numTrackV))+4;
		for my $udeIndex (0 .. $#udEdges) {
			my $fromMetal = (split /[a-z]/, $udEdges[$udeIndex][1])[1]; # 1:metal 2:row 3:col
			...
			if($fromMetal == 3 && $toMetal == 4 && ($fromRow == $row && $toRow == $row) && ($fromCol == $toCol && $toCol == $col)){
				if($iffirst == 1){
					# this is the first row entering this: establish assert
					print $out "(assert (!(= M2_TRACK_$row (or";
					$iffirst = 0;
					$ifwritten = 1;
				}
...
foreach my $key(keys %h_extnets){
		...
		for my $col (@allcols){
			...
			for my $udeIndex (0 .. $#udEdges) {
				...
				if($fromMetal == 3 && $toMetal == 4 && ($fromRow == $row && $toRow == $row) && ($fromCol == $toCol && $toCol == $col)){
					for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
						if($iffirst == 1){
							# this is the first row entering this: put assert at front
							print $out "(assert (!(= N".$key."_M2_TRACK_$row (or";
							$iffirst = 0;
							$ifwritten = 1;
						}
	...
	# this is strictly M2 (M4) tracks
	my $temp_numTrackH = $map_numTrackH{4};
	for my $row_idx (0 .. $temp_numTrackH -3){
		my $row = $map_metal_to_rows{$metal}[$row_idx];
...
# 12/7/2022 UPDATE FLAG 5
my $isPRT_P = 0;
my $isPRT_N = 0;
# YW: Not actively used. Skip for now?
if($Partition_Parameter == 2){
	my $numP = scalar @inst_group_p;
	if($numP > 0){
		# Since partition info is not provided in pinlayout file, this is not used
...
print $out "(assert (!(= COST_SIZE (max COST_SIZE_P COST_SIZE_N)) :named c$CONSTRAINT_ID))\n";
$CONSTRAINT_ID += 1;
...
# 12/07/2022 UPDATE FLAG 6
# YW: no change for now. Should I incoporate CPP
print $out "(minimize COST_SIZE)\n";
	...
	for my $col_idx (0 .. $temp_numTrackV-1) {
		...
		if ($col_idx % 2 == 0) {
			...
			if ($col_idx == 0) {
				...
				print $out " (ite (= COST_Pin_C$col true) (_ bv2 ".(length(sprintf("%b", $max_numTrackV))+4).") (_ bv0 ".(length(sprintf("%b", $max_numTrackV))+4)."))";
			} else {
	...
	for my $col_idx (0 .. $temp_numTrackV-1) {
		...
		if ($col_idx % 2 == 0) {
			...
			if ($col_idx == 0) {
				...
				# print $out " (ite (= COST_Pin_C$col true) (_ bv1 ".(length(sprintf("%b", $numTrackV))+4).") (_ bv0 ".(length(sprintf("%b", $numTrackV))+4)."))";
				print $out " (ite (= COST_Pin_C$col true) (_ bv1 ".(length(sprintf("%b", $max_numTrackV))+4).") (_ bv0 ".(length(sprintf("%b", $max_numTrackV))+4)."))";
			} else {
				# print $out " (ite (= COST_Pin_C$col true) (_ bv1 ".(length(sprintf("%b", $numTrackV))+4).") (_ bv0 ".(length(sprintf("%b", $numTrackV))+4)."))";
				print $out " (ite (= COST_Pin_C$col true) (_ bv1 ".(length(sprintf("%b", $max_numTrackV))+4).") (_ bv0 ".(length(sprintf("%b", $max_numTrackV))+4)."))";
			}
...
if ($PE_Parameter == 2) {
	...
	# 1/20 why M3
	my $numEdges = $map_numTrackV{3}*$map_numTrackH{3};
	print $out "(minimize (bvadd";
			...
			for my $extnetIndex (0 .. $#outerPins) {
				...
				if ($outerPins[$extnetIndex][1] eq $nets[$netIndex][0]) {
					...
					for my $row_idx (0 .. $temp_numTrackH-4) {
						...
						my $row = $map_metal_to_rows{$metal}[$row_idx];
						my $row_b = $map_metal_to_rows{$metal}[$row_idx+1];
...
#			}
		...
		$limit_obj = 75;
		print $out $str_opt;
		# Assume tocol is at metal 4
		# for my $col(0 .. ($numTrackV-1)/2-1){
		my $temp_numTrackV = $map_numTrackV{4};
		for my $col_idx (0 .. ($temp_numTrackV-1)/2-1) {
			my $col = $map_metal_to_cols{4}[$col_idx];
			...
			for my $udeIndex (0 .. $#udEdges) {
				...
				my $fromCol = (split /[a-z]/, $udEdges[$udeIndex][1])[3]; # 1:metal 2:row 3:col
				my $toCol = (split /[a-z]/, $udEdges[$udeIndex][2])[3];
				if($fromMetal == $toMetal && $toMetal == 4 && ($fromCol == $col || $toCol == $map_metal_to_cols{4}[($temp_numTrackV-1-$col_idx)])){
...
#		$idx_obj = 0;
	...
	my $temp_numTrackH = $map_numTrackH{4};
	for my $row_idx (0 .. $temp_numTrackH-3) {
		my $row = $map_metal_to_rows{4}[$row_idx];
		...
		print $out " (ite (= M2_TRACK_$row true) (_ bv1 ".(length(sprintf("%b", $max_numTrackH))).") (_ bv0 ".(length(sprintf("%b", $max_numTrackH)))."))";
		$idx_obj++;
	}
	print $out "))\n";
	...
	for my $col_idx (0 .. ($temp_numTrackV-1)/2-1) {
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			if($toMetal == 4 && ($fromCol == $col || $toCol == $map_metal_to_cols{4}[($temp_numTrackV-1-$col_idx)]) && $fromMetal == 4){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
	...
	my $temp_numTrackV = $map_numTrackV{4};
	for my $col_idx (0 .. ($temp_numTrackV-1)/2) {
		my $col = $map_metal_to_cols{4}[$col_idx];
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			my $fromCol = (split /[a-z]/, $udEdges[$udeIndex][1])[3]; # 1:metal 2:row 3:col
			my $toCol = (split /[a-z]/, $udEdges[$udeIndex][2])[3];
			if($fromMetal!=$toMetal && ($toMetal>=2 && $toMetal<=3) && ($fromCol == $col || $fromCol == $map_metal_to_cols{4}[($temp_numTrackV-1-$col_idx)])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
	...
	for my $col_idx (0 .. ($temp_numTrackV-1)/2-1) {
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			if($fromMetal==$toMetal && ($toMetal>=2 && $toMetal <= 3) 
				&& ($fromCol == $col || $toCol == $map_metal_to_cols{4}[($temp_numTrackV-1-$col_idx)])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
			}
			elsif($fromMetal==$toMetal && ($toMetal>=2 && $toMetal <= 3) 
				&& ($fromCol==$toCol && $fromCol==$map_metal_to_cols{4}[($temp_numTrackV-1)/2]) && ($col == $map_metal_to_cols{4}[($temp_numTrackV-1)/2-1])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
	...
	for my $col_idx (0 .. ($temp_numTrackV-1)/2-1) {
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			my $fromCol = (split /[a-z]/, $udEdges[$udeIndex][1])[3]; # 1:metal 2:row 3:col
			my $toCol = (split /[a-z]/, $udEdges[$udeIndex][2])[3];
			if($toMetal>=2 && ($fromCol == $col || $toCol == $map_metal_to_cols{4}[($temp_numTrackV-1-$col)])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
				...
				&& ($fromCol==$toCol && $fromCol==$map_metal_to_cols{4}[($temp_numTrackV-1)/2]) 
				&& ($col == $map_metal_to_cols{4}[($temp_numTrackV-1)/2-1])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
	...
	for my $col_idx (0 .. ($temp_numTrackV-1)/2) {
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			my $toCol = (split /[a-z]/, $udEdges[$udeIndex][2])[3];
			if($fromMetal!=$toMetal && ($toMetal>=2 && $toMetal<=3) 
				&& ($fromCol == $col || $fromCol == $map_metal_to_cols{4}[($temp_numTrackV-1-$col_idx)])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
	...
	for my $col_idx (0 .. ($temp_numTrackV-1)/2 - 1) {
		...
		for my $udeIndex (0 .. $#udEdges) {
			...
			my $fromCol = (split /[a-z]/, $udEdges[$udeIndex][1])[3]; # 1:metal 2:row 3:col
			my $toCol = (split /[a-z]/, $udEdges[$udeIndex][2])[3];
			if($fromMetal==$toMetal && ($toMetal>=2 && $toMetal <= 3) && ($fromCol == $col || $toCol == $map_metal_to_cols{4}[($temp_numTrackV-1-$col_idx)])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
			}
			elsif($fromMetal==$toMetal && ($toMetal>=2 && $toMetal <= 3) && ($fromCol==$toCol && $fromCol==$map_metal_to_cols{4}[($temp_numTrackV-1)/2]) && ($col == $map_metal_to_cols{4}[($temp_numTrackV-1)/2-1])){
				if(exists($h_var{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
					print $out " (ite (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] true) (_ bv$udEdges[$udeIndex][3] 32) (_ bv0 32))";
					$idx_obj++;
				}
