...
while($isEnd == 0){
										...
										if(!exists($h_tmp{$tmp_vname1.$tmp_vname2})){
											if(!exists($h_assign{"M_$tmp_vname1\_$tmp_vname2"})){
												...
												if ($col_idx == 0) {
													...
													$str.="(assert (ite (bvsge COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 true) (= 1 1)))\n";
												} else {
													# if bitvector signed COST SIZE >= col
													...
													$str.="(assert (ite (bvslt COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 false) (= 1 1)))\n";
												}
											...
											if(!exists($h_tmp{$tmp_vname1.$tmp_vname2})){
												if(!exists($h_assign{"M_$tmp_vname1\_$tmp_vname2"})){
													...
													if ($col_idx == 0) {
														...
														# then assert M_v#_vR# = true
														# else do nothing?
														print("(assert (ite (bvsge COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 true) (= 1 1)))\n");
														$str.="(assert (ite (bvsge COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 true) (= 1 1)))\n";
													} else {
														# if bitvector signed COST SIZE >= col
														# then assert M_v#_vR# = false
														# else do nothing?
														print("(assert (ite (bvslt COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 false) (= 1 1)))\n");
														$str.="(assert (ite (bvslt COST_SIZE (_ bv".$map_metal_to_cols{$metal}[($col_idx>0?$col_idx-1:$col_idx)]." $len)) (= M_$tmp_vname1\_$tmp_vname2 false) (= 1 1)))\n";
													}
													$h_tmp{$tmp_vname1.$tmp_vname2} = 1;
													cnt("l", 1);
	...
	for my $udeIndex (0 .. $#udEdges) {
		...
		if($minWidth>2 && $toCol > $minWidth-2){
			...
			if(!exists($h_assign{"M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2]"})){
				...
				# else, do nothing
				$str.="(assert (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= M_$udEdges[$udeIndex][1]_$udEdges[$udeIndex][2] false) (= true true)))\n";
				cnt("l", 1);
				cnt("l", 1);
				cnt("c", 1);
			}
	...
	for my $vEdgeIndex (0 .. $#virtualEdges) {
		...
		if($minWidth>2 && $toCol > $minWidth-2){
			...
			if(!exists($h_assign{"M_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"})){
				...
				# else, do nothing
				$str.="(assert (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= M_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)))\n";
				cnt("l", 1);
				cnt("l", 1);
				cnt("c", 1);
			}
			...
			if($minWidth>2 && $toCol > $minWidth-2){
				...
				if($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3]){
					...
					if(!exists($h_assign{"N$nets[$netIndex][1]_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"})){
						...
						# then Net NETID_E_#vertex1_#vertex2 = False
						# else, do nothing
						$str.="(assert (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= N$nets[$netIndex][1]_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)))\n";
						cnt("l", 1);
						cnt("l", 1);
						cnt("c", 1);
					}
					...
					for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {
						...
						if($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]){
							...
							if(!exists($h_assign{"N$nets[$netIndex][1]_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]"})){
								...
								# minus 2 because SDB?
								$str.="(assert (ite (bvsle COST_SIZE (_ bv".$map_metal_to_cols{$toMetal}[($toCol_idx-2)]." $len)) (= N$nets[$netIndex][1]_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2] false) (= true true)))\n";
								cnt("l", 1);
								cnt("l", 1);
								cnt("c", 1);
							}
				...
				for my $netIndex (0 .. $#nets) {
					...
					for my $commodityIndex (0 .. $nets[$netIndex][4]-1) {	
						for my $vEdgeIndex (0 .. $#virtualEdges) {
							...
							if ($virtualEdges[$vEdgeIndex][2] eq "pinSON" && ( ($col_idx - $toCol == $dint) || ($toCol - $col_idx == $dint) ) && ($virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][3] || $virtualEdges[$vEdgeIndex][2] eq $nets[$netIndex][5][$commodityIndex]) ) {
								my $tmp_var = "N$nets[$netIndex][1]\_C$commodityIndex\_E_$virtualEdges[$vEdgeIndex][1]_$virtualEdges[$vEdgeIndex][2]";
								push (@tmp_arr, $tmp_var);
								#$tmp_str.=" (= $tmp_var false) ";
								#$valid = 1;
	...
	$str.=";Set Initial Value for Shared Gate/S/D Source to sinks Pins of P/N FET in the same column\n";
		...
		for my $p_col_idx (1 .. $temp_numTrackV-1){
			...
			for my $netIndex (0 .. $#nets) {
				for my $vEdgeIndex (0 .. $#virtualEdges) {
					...
					if (($toCol < $p_col || $toCol >= $p_col+$real_w) && $toCol > 0  && $toCol < $temp_numTrackV-2 && $vEdge_Instidx == $i) {
						my $isInNet = 0;
	...
	for my $i (0 .. $numInstance - 1) {
		...
		my $real_w = (2*$tmp_finger[0]+1) * $mapMP{"1"};

		# iterate through width index (not real width)
		for my $pin_subidx_idx (1 .. $w) {
			...
			for (my $p_col_idx = 1; $p_col_idx < $temp_numTrackV; $p_col_idx = $p_col_idx+2) {
				...
				for my $vEdgeIndex (0 .. $#virtualEdges) {
					...
					if($virtualEdges[$vEdgeIndex][2] ne $keySON){
						...
						if (($toCol >= $p_col && $toCol < $p_col+$real_w) && $toCol > 0  && $toCol < $temp_numTrackV-2 && $vEdge_Instidx == $i && $vEdge_pin_subidx == $pin_subidx_idx) {
