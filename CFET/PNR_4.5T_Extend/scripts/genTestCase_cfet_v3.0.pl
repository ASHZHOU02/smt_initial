...
my $ARGC        = @ARGV;
my $workdir     = getcwd();
my $outdir      = "$workdir/pinLayouts_cfet_v3.0";
my $infile      = "";
...
my @mapMP = ($ENV{M1_MP}, $ENV{M2_MP}, $ENV{M3_MP}, $ENV{M4_MP});
my @mapOffset = ($ENV{M1_OFFSET}, $ENV{M2_OFFSET}, $ENV{M3_OFFSET}, $ENV{M4_OFFSET});
...
# sync up with largest offset, default to 1
my $sizeOffset = 1;
# require input config file
if ($ARGC != 2) {
    print "\n*** Error:: Wrong CMD";
