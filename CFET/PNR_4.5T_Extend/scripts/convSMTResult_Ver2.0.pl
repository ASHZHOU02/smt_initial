...
close($in);
# Number of Routing Clip = Total number of horizontal tracks -  2 power rail
...
#print "VIA $fromM $toM $fromR $fromC\n";
    ...
    if ($line =~ /^.*\(define-fun N(\S+)_E_m(\d+)r(\d+)c(\d+)_m(\d+)r(\d+)c(\d+)/) {
		...
		# 1/22 if netID not found error occur again, debug this
		# print "debug: $line\n";
		$line = <$in>;
		chomp($line);
		$line =~ s/^\s+(\S+)\)/$1/g;
    ...
    if ($line =~ /^.*\(define-fun COST_SIZE /) {
		...
		if($line > $cost_placement){
			$cost_placement = $line + 2 * int($ENV{M1_MP});
		}
	}
...
sub printResult{
	...
	for my $i(0 .. (scalar @final_via) -1){
		...
		if($final_via[$i][0] == 3 && $final_via[$i][1] == 4){
			# 1/22 netid is retrieved from h_net
			my $netID = -1;
			$netID = $h_net{$final_via[$i][0]."_".$final_via[$i][1]."_".$final_via[$i][2]."_".$final_via[$i][3]."_".$final_via[$i][2]."_".$final_via[$i][3]};

			my $temp_str = "m".$final_via[$i][0]."r".$final_via[$i][2]."c".$final_via[$i][3]."_m".$final_via[$i][1]."r".$final_via[$i][2]."c".$final_via[$i][3];
			print "NETID: $netID => $temp_str\n";
			# [fromMetalLayer, toMetalLayer, Row, Col]
