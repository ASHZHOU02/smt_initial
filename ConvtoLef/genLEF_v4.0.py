...
class Metal:
    ...
    # missing function __lt__, but this is not even used in probe3.0
    def dump(self):
    ...
    def getLefStr(self, techInfo):
        ...
        retStr += "        RECT %.3f %.3f %.3f %.3f ;\n" \
            % ( techInfo.getLx(val=self.fromCol, layer=self.layer),
                techInfo.getLy(val=self.fromRow, layer=self.layer), \
                techInfo.getUx(val=self.toCol, layer=self.layer), \
                techInfo.getUy(val=self.toRow, layer=self.layer))
        return retStr

    # TODO parse layer orientation
    @classmethod
    def getLayerOrient(layer):
        """
        Returns layer orientation
        # Odd Layers: Vertical Direction   
        # Even Layers: Horizontal Direction
        """
        if layer % 2 == 0:
            return "HORIZONTAL"
        else:
    ...
    def getLefLayerStr(self, techInfo):
        ...
        resStr += "LAYER metal%d\n  TYPE ROUTING ;\n  WIDTH %.3f ;\n  SPACING %.3f ;\n  PITCH %.3f ;\n  DIRECTION %s ;\n\nEND metal%d\n" \
          % (self.layer, \
            techInfo.getWidth(self.layer)/1000, \
            techInfo.getSpacing(self.layer)/1000, \
            techInfo.getPitch(self.layer)/1000, \
            Metal.getLayerOrient(self.layer), \
            self.layer)
        return resStr
...
class Via:
    ...
    def getLefStr(self, techInfo):
        # should via only come from layer 0? no, this is a default val
        # x should be respecting vertical MP
        # y should be respecting horizontal MP
        if Metal.getLayerOrient(self.fromMetal) == "HORIZTONAL" and Metal.getLayerOrient(self.fromMetal) == "VERTICAL":
            hori_metal = self.fromMetal
            vert_metal = self.toMetal
        elif Metal.getLayerOrient(self.fromMetal) == "VERTICAL" and Metal.getLayerOrient(self.fromMetal) == "HORIZTONAL":
            hori_metal = self.toMetal
            vert_metal = self.fromMetal
        else:
            raise Exception("[ERROR VIA] Horzontal/Vertical Layer not found.")
        
        retStr = ""
        retStr += "        RECT %.3f %.3f %.3f %.3f ;\n" \
            % ( techInfo.getLx(self.y, vert_metal), \
                techInfo.getLy(self.x, hori_metal), \
                techInfo.getUx(self.y, vert_metal), \
                techInfo.getUy(self.x, hori_metal))
        return retStr
...
class TechInfo:
    def __init__(self, numCpp, numTrack, metalPitchM1, metalPitchM2, metalPitchM3, metalPitchM4, cppWidth, siteName, bprFlag, mpoFlag):
        self.numCpp = int(numCpp)
        self.numTrack = int(numTrack)
        self.metalPitchM1 = int(metalPitchM1)
        self.metalPitchM2 = int(metalPitchM2)
        self.metalPitchM3 = int(metalPitchM3)
        self.metalPitchM4 = int(metalPitchM4)
    ...
    def update(self, isMaxCellWidthUpdate):
        ...
        self.metalWidthM1 = int(self.metalPitchM1/2)
        self.metalWidthM2 = int(self.metalPitchM2/2)
        self.metalWidthM3 = int(self.metalPitchM3/2)
        self.metalWidthM4 = int(self.metalPitchM4/2)
        # Buried Power Rail
        ...
        self.cellHeight = (self.realTrack) * self.metalPitchM2
    ...
    def getLx(self, val, layer):
        if layer == 3:
            ...
            return (self.cppWidth/2 \
              + val * (self.cppWidth/2) - TechInfo.getWidth(layer)/2)/1000.0 - 0.009
        else:
            return (self.cppWidth/2 \
              + val * (self.cppWidth/2) - TechInfo.getWidth(layer)/2)/1000.0

    # BPRMODE with METAL1 / METAL2 should shift coordinates by + metalPitch/2.0
    def getLy(self, val, layer):
        if self.bprFlag == BprMode.BPR:
            offset = 3 * TechInfo.getPitch(layer)/4
        if self.bprFlag == BprMode.METAL1 or self.bprFlag == BprMode.METAL2:
            offset = 3 * TechInfo.getPitch(layer)/2
        calVal = (offset \
          + val * TechInfo.getPitch(layer) - TechInfo.getWidth(layer)/2)/1000.0
    ...
    @classmethod
    def getPitch(layer):
        if   layer == 1 : return TechInfo.metalPitchM1
        elif layer == 2 : return TechInfo.metalPitchM2
        elif layer == 3 : return TechInfo.metalPitchM3
        elif layer == 4 : return TechInfo.metalPitchM4
        raise Exception("[ERROR TechInfo] Layer info not found")

    @classmethod
    def getWidth(layer):
        if   layer == 1 : return TechInfo.metalWidthM1
        elif layer == 2 : return TechInfo.metalWidthM2
        elif layer == 3 : return TechInfo.metalWidthM3
        elif layer == 4 : return TechInfo.metalPitchM4
        raise Exception("[ERROR TechInfo] Layer info not found")

    def getSpacing(self, layer):
        return TechInfo.getPitch(layer) - TechInfo.getWidth(layer)
    ...
    def getLibraryName(self):
        # TODO: which MP should we take as filename
        return "%dT_%dF_%dCPP_%dMP_%s_%s_%s" \
            % (self.realTrack, self.numFin, self.cppWidth, self.metalPitchM1, \
...
def main():
    ...
    if len(sys.argv) <= 1:
        print("Usage:   python generate.py <siteName>\n\n")
        print("         metalPitch: {int}, {int}, {int}, {int}")
        print("         cppWidth  : int")
    ...
    tech = TechInfo(
      numCpp=0, 
      numTrack=0, 
      metalPitchM1=os.environ['M1_MP'],
      metalPitchM2=os.environ['M2_MP'],
      metalPitchM3=os.environ['M3_MP'],
      metalPitchM4=os.environ['M4_MP'],
      cppWidth=os.environ['CPPWidth'], 
      siteName=sys.argv[3], 
      bprFlag=BprMode.NONE,
      mpoFlag=GetMpoFlag(os.environ['MPOMode'])
        )
